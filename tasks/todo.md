...
## Round 2 — the open items (2026-09-16)

Picked from "Next steps" above: the three that are unblocked here. Deploy, a device run and an age
signal wait on accounts and hardware this environment does not have.

- [x] **A — DOD-02 remainder: a photo and a voice note end to end.** New journey
      `e2e/journeys/13-media-messages.spec.ts`: A attaches a generated PNG through the composer's
      `+` → Photo or video (the real file chooser) and records a voice note over Chromium's fake
      microphone (`MediaRecorder`, stop-and-send); B's open thread shows both, and the bytes come
      back through the signed media route (`img.naturalWidth > 0`, `audio.readyState ≥ 1`). First
      journey to put real bytes through Storage. Fixture `e2e/fixtures/media.ts` makes the PNG.
- [x] **B — Milestone 6 gate: the map shows Lives by area.** New journey
      `e2e/journeys/14-live-pins.spec.ts`: A (geolocated in the Mission, `Use my location` →
      `context_set`) starts a group room and opens it to Neighborhood; Ben (seed, Mission context)
      switches Earth to Neighborhood and sees the pin, named for A (0998: never the private group's
      name), with `map_objects` positioning it at the Mission centroid.
...
1. Recipients could not view any photo or voice message, on either client. `useMediaUrl` signed
   Storage URLs as the viewer, and the `media` / `voice` buckets admit their owner only (0997):
   the sender saw the photo, every recipient got a 404. Fix: `mediaRouteUrl()` in `@earth/api`
   — the same `/api/media/<bucket>/<key>` shape `earth.media_url()` gives post media — used by
   both clients' `useMediaUrl`.
2. That route authenticates by bearer only, and an `<img>`/`<audio>`/`<video>` cannot send one,
   so on the web it was unreachable from the one place it serves (post photos included). Fix:
   the web mount presents the `@supabase/ssr` session cookie as the bearer for a bare `GET` of
   that route alone (`apps/web/lib/server/cookie-bearer.ts`); mobile sends the token as request
   headers on its native media sources (`apps/mobile/features/media/requestHeaders.ts`).
3. The route then answered `internal` in every environment: `ServerDeps.storage` is optional and
   the web mount's client adapter never forwarded `storage`, so nothing ever signed. Fix:
   `supabaseClientFrom` forwards it; the fake carries one so `handler.test.ts` proves the full
   chain (grant as the viewer → signature by the service role → private `302`).
