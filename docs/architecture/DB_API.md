...
| RPC | Caller | Behavior |
| --- | --- | --- |
| `report_create(target_type, target_id, reason report_reason, details)` | human or guest | Guests may report only rooms/participants of their room. A Human reports what they can see, which includes everyone holding a seat in a room with them: another participant as `target_type='human'` (`earth.human_reportable_by`, shared-room branch) and a Guest as `target_type='guest'` with the guest session id (`earth.room_visible_to`) — the two targets the participants sheet of spec §81 sends. Rate limited. |
...
`app_settings(key text pk, value text)` — `public_storage_base_url`, `room_grace_seconds`, `web_origin`, `environment`, `minimum_age_policy` (spec §84: `18_plus` at launch; the gate reads anything unrecognised as `18_plus`). Readable by all.
