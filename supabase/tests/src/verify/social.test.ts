...
async function liveRooms(db: TestDb, scope: string, as: RoleSpec): Promise<LiveCandidateRow[]> {
  const list = await db.rpc<{ candidates: LiveCandidateRow[] }>(
    'live_candidates',
    { scope, area_id: null },
    as,
  )
...
describe('social invariants — adversarial verification (spec §128)', () => {
  ...
  describe('group member is not automatically Friend', () => {
    ...
    it('profile and member lists report shared membership, never friendship', async () => {
      ...
      expect(seenByMilo.relationship).toMatchObject({
        isFriend: false,
        friendRequest: 'none',
        isFollowing: false,
        isFollowedBy: false,
      ...
      const group = GroupDetailDtoSchema.parse(
        await db.rpc('group_get', { group_id: crew.groupId }, milo.as),
      )
      ...
      const again = GroupDetailDtoSchema.parse(
        await db.rpc('group_get', { group_id: crew.groupId }, milo.as),
      )
    ...
    it('a friends post is invisible to a group-mate on every read path; a friend sees it', async () => {
      const post = await createPost(db, olive, {
        audience: 'friends',
        text: 'olive friends only quokka',
      })
      ...
      expect(
        await rowsAs(db, milo.as, 'select id from public.posts where id = $1', [post.post.id]),
      ).toEqual([])
      await db.expectError(
        db.rpc('post_reaction_set', { post_id: post.post.id, reaction_type: 'like' }, milo.as),
        'post_not_found',
      )
      await db.expectError(
        db.rpc(
          'post_create',
          { type: 'text', text: 'reply', audience: 'friends', parent_post_id: post.post.id },
          milo.as,
        ),
        'post_not_found',
      )
      expect((await search(db, milo.as, 'quokka')).posts.map((p) => p.post.id)).not.toContain(
        post.post.id,
      )
      expect((await search(db, fiona.as, 'quokka')).posts.map((p) => p.post.id)).toContain(
        post.post.id,
      )
      ...
      expect(fionasFriends.candidates.find((c) => c.id === post.post.id)).toMatchObject({
        relationship: 'friend',
      })
    })
    ...
    it('a friends Live of a group-mate is invisible, unjoinable and unannounced; a group Live is not a friends Live', async () => {
      ...
      expect(
        await rowsAs(db, milo.as, 'select id from public.rooms where id = $1', [roomId]),
      ).toEqual([])
      expect(
        await rowsAs(db, milo.as, 'select id from public.room_participants where room_id = $1', [
          roomId,
        ]),
      ).toEqual([])
      expect(
        await feed(db, 'friends', milo.as).then((f) => f.candidates.map((c) => c.id)),
      ...
      expect(fionasView.participants.map((p) => [p.humanId, p.relationToViewer])).toEqual([
        [olive.humanId, 'friend'],
      ])
      ...
      expect(milosView.participants.map((p) => [p.humanId, p.relationToViewer])).toEqual([
        [olive.humanId, 'shared_group'],
      ])
      ...
      expect(
        await errorCode(
          db.rpc(
            ...
            shareArgs({ audienceId: milo.humanId, position: POINTS.northBeach }),
            olive.as,
          ),
        ),
      ).toBe('forbidden')
      const toFriend = await createShare(db, olive, {
        audienceId: fiona.humanId,
        position: POINTS.northBeach,
      })
      const toGroup = await createShare(db, olive, {
        audienceType: 'group',
    ...
    it('a hidden profile stays hidden from a group-mate (friends only)', async () => {
      ...
      expect((await search(db, milo.as, 'Hattie')).people.map((p) => p.humanId)).not.toContain(
        hattie.humanId,
      )
  ...
  describe('Friend is not Follow', () => {
    ...
    it('follow writes one directional edge and no friendship state', async () => {
      const followed = RelationshipChangeDtoSchema.parse(
        await db.rpc('follow_set', { target_human_id: ben.humanId, following: true }, ann.as),
      )
      ...
      expect(bensView.relationship).toMatchObject({
        isFriend: false,
        isFollowing: false,
        isFollowedBy: true,
        friendRequest: 'none',
      ...
      expect(annsView.relationship).toMatchObject({
        isFriend: false,
        isFollowing: true,
        isFollowedBy: false,
      })
      ...
      expect(
        (await search(db, ann.as, 'Ben')).people.find((p) => p.humanId === ben.humanId),
      ).toMatchObject({ isFollowing: true, isFriend: false })
    })

    it('a follower gets nothing friend-gated: friends posts, friends Lives, friend_live, friend shares', async () => {
      const post = await createPost(db, ben, {
        audience: 'friends',
        text: 'ben friends only narwhal',
      })
      ...
      expect(annsFriends.candidates.find((c) => c.id === world.post.id)).toMatchObject({
        relationship: 'follow',
      })
      ...
      expect(
        await rowsAs(db, ann.as, 'select id from public.rooms where id = $1', [roomId]),
      ).toEqual([])
      ...
      expect(
        await errorCode(
          db.rpc(
            'location_share_create',
            shareArgs({ audienceId: ann.humanId, position: POINTS.northBeach }),
            ben.as,
          ),
        ),
      ).toBe('forbidden')
      expect(
        await errorCode(
          db.rpc(
            'location_share_create',
            shareArgs({ audienceId: ben.humanId, position: POINTS.northBeach }),
            ann.as,
          ),
        ),
      ).toBe('forbidden')
    ...
    it('friendship writes friend rows only: no follow edge appears, in either direction', async () => {
      ...
      const accepted = RelationshipChangeDtoSchema.parse(
        await db.rpc('friend_request_accept', { source_human_id: ben.humanId }, cy.as),
      )
      ...
      expect((await profile(db, cy.handle, ben.as)).relationship).toMatchObject({
        isFriend: true,
        isFollowing: false,
        isFollowedBy: false,
      })
      expect((await profile(db, ben.handle, cy.as)).relationship).toMatchObject({
        isFriend: true,
        isFollowing: false,
        isFollowedBy: false,
      })
      expect((await profile(db, ben.handle, cy.as)).counts).toMatchObject({
        friends: 1,
        followers: 1,
        following: 0,
      })
      expect(
        (await search(db, ben.as, 'Cy')).people.find((p) => p.humanId === cy.humanId),
      ).toMatchObject({ isFriend: true, isFollowing: false })
      expect(
        (await notificationsFor(db, cy))
          .filter((n) => n.actor_human_id === ben.humanId)
          .map((n) => n.type),
      ).toEqual(['friend_request'])
      expect(
        (await notificationsFor(db, ben))
          .filter((n) => n.actor_human_id === cy.humanId)
          .map((n) => n.type),
      ).toEqual(['friend_accepted'])
    })
    ...
    it('request, accept, remove and decline never touch follow edges; unfollow never touches friendship', async () => {
      ...
      expect((await profile(db, ben.handle, ann.as)).relationship).toMatchObject({
        isFriend: true,
        isFollowing: true,
      })
      ...
      const unfollowed = RelationshipChangeDtoSchema.parse(
        await db.rpc('follow_set', { target_human_id: ben.humanId, following: false }, ann.as),
      )
      ...
      const declined = RelationshipChangeDtoSchema.parse(
        await db.rpc('friend_request_decline', { source_human_id: ann.humanId }, cy.as),
      )
      ...
      expect((await profile(db, cy.handle, ann.as)).relationship).toMatchObject({
        isFriend: false,
        friendRequest: 'sent',
        isFollowing: false,
        isFollowedBy: true,
  ...
  describe('familiar_private is hidden from its target', () => {
    ...
    it('the target never reads the row; the source does; nobody can write rows directly', async () => {
      ...
      for (const as of [mara.as, tom.as]) {
        await expect(
          db.asRole(as, (c) =>
            c.query(
              "insert into public.relationships (source_human_id, target_human_id, type) values ($1, $2, 'familiar_private')",
              [tom.humanId, mara.humanId],
            ),
          ),
        ).rejects.toMatchObject({ code: PERMISSION_DENIED })
        await expect(
          db.asRole(as, (c) => c.query('update public.relationships set type = $1', ['follow'])),
        ).rejects.toMatchObject({ code: PERMISSION_DENIED })
        await expect(
          db.asRole(as, (c) => c.query('delete from public.relationships')),
        ).rejects.toMatchObject({ code: PERMISSION_DENIED })
      }
    })
    ...
    it('profiles expose no familiar flag in either direction', async () => {
      ...
      expect(byTarget.relationship).toEqual({
        isSelf: false,
        isFriend: false,
        friendRequest: 'none',
        isFollowing: false,
    ...
    it('room participant relations: the source sees `familiar`, the target sees `other`', async () => {
      ...
      expect(listed?.participants.map((p) => [p.humanId, p.relationToViewer])).toEqual([
        [mara.humanId, 'other'],
      ])
      await joinRoom(db, roomId, tom, 'watching')
      const marasView = await getRoom(db, roomId, mara.as)
      expect(marasView.participants.find((p) => p.humanId === tom.humanId)?.relationToViewer).toBe(
        'familiar',
      )
      const tomsView = await getRoom(db, roomId, tom.as)
      expect(tomsView.participants.find((p) => p.humanId === mara.humanId)?.relationToViewer).toBe(
        'other',
      )
      expect(tomsView.participants.find((p) => p.humanId === tom.humanId)?.relationToViewer).toBe(
        'self',
      )
    })

    it('a block keeps the private edge private (it survives, still invisible to the target)', async () => {
      const blocked = await db.rpc<{ isBlocked: boolean }>(
        'block_set',
        { target_human_id: tom.humanId },
        mara.as,
      )
  ...
  describe('blocks override every surface — sequences beyond the straight line', () => {
    ...
    beforeAll(async () => {
      ...
      const result = await db.rpc<{ isBlocked: boolean; isFriend: boolean }>(
        'block_set',
    ...
    it('a watching seat is still a seat: the blocked pair never shares a live room, in either order', async () => {
      ...
      await db.expectError(
        db.rpc(
          'room_join',
          { room_id: coffee, media_state: 'camera', consent_level: 'friends' },
          bob.as,
        ),
        'room_not_found',
      )
      await db.expectError(
        db.rpc(
          'room_join',
          { room_id: coffee, media_state: 'watching', consent_level: 'invited' },
          bob.as,
        ),
        'room_not_found',
      )
      expect(
        await rowsAs(db, bob.as, 'select id from public.rooms where id = $1', [coffee]),
      ).toEqual([])
      const link = await createRoomInvite(db, coffee, carol)
      await db.expectError(
        db.rpc(
          'room_invite_join',
          { token: link.token, media_state: 'watching', consent_level: 'invited' },
          bob.as,
        ),
        'room_not_found',
      )
      expect(
        (await getRoom(db, coffee, alice.as)).participants.map((p) => p.humanId).sort(),
      ).toEqual([alice.humanId, carol.humanId].sort())
      ...
      expect(bobsView.participants.map((p) => p.humanId).sort()).toEqual(
        [bob.humanId, carol.humanId].sort(),
      )
      ...
      expect(
        await rowsAs(db, alice.as, 'select id from public.rooms where id = $1', [coffee]),
      ).toEqual([])
      ...
      expect(
        (await getRoom(db, coffee, carol.as)).participants.map((p) => p.humanId).sort(),
      ).toEqual([bob.humanId, carol.humanId].sort())
    ...
    it('an invited seat in a direct room counts too: the link never seats the blocked Human, and the preview names nobody blocked', async () => {
      ...
      const direct = RoomDtoSchema.parse(
        (
          await db.rpc<{ room: unknown }>(
            'room_start',
      ...
      expect(direct.participants.map((p) => [p.humanId, p.status])).toEqual(
        expect.arrayContaining([
          [carol.humanId, 'active'],
          [alice.humanId, 'invited'],
        ]),
      )
      ...
      await db.expectError(
        db.rpc(
          'room_invite_join',
          { token: link.token, media_state: 'camera', consent_level: 'invited' },
          bob.as,
        ),
        'room_not_found',
      )
      expect(
        await rowsAs(db, bob.as, 'select id from public.rooms where id = $1', [direct.id]),
      ).toEqual([])
      const preview = RoomInvitePreviewDtoSchema.parse(
        await db.rpc('room_invite_preview', { token: link.token }, bob.as),
      )
      ...
      expect(alicesView.participants.map((p) => p.humanId).sort()).toEqual(
        [alice.humanId, carol.humanId].sort(),
      )
      expect(
        await rowsAs(db, bob.as, 'select id from public.room_participants where room_id = $1', [
          direct.id,
        ]),
      ).toEqual([])
    ...
    it('an invite link to the blocker’s own room reveals nothing and seats nobody; third parties are unaffected', async () => {
      ...
      const forBob = RoomInvitePreviewDtoSchema.parse(
        await db.rpc('room_invite_preview', { token: link.token }, bob.as),
      )
      ...
      const forCarol = RoomInvitePreviewDtoSchema.parse(
        await db.rpc('room_invite_preview', { token: link.token }, carol.as),
      )
      expect(forCarol.participants.map((p) => p.displayName)).toEqual(['Alice'])
      expect(forCarol.invitedByDisplayName).toBe('Alice')
      const forVisitor = RoomInvitePreviewDtoSchema.parse(
        await db.rpc('room_invite_preview', { token: link.token }, 'visitor'),
      )
    ...
    it('inside a shared group, replying to and reacting on the blocked Human’s messages is a direct interaction and is refused both ways', async () => {
      ...
      expect(
        await errorCode(
          db.rpc('message_reaction_toggle', { message_id: fromAlice, reaction: '👍' }, bob.as),
        ),
      ).toBe('blocked')
      ...
      expect(
        await errorCode(
          db.rpc('message_reaction_toggle', { message_id: fromBob, reaction: '👍' }, alice.as),
        ),
      ).toBe('blocked')
    ...
    it('temporary group chats cannot be created across the block by either side; a third party’s chat never notifies across it', async () => {
      await db.expectError(
        db.rpc('conversation_group_create', { human_ids: [alice.humanId, carol.humanId] }, bob.as),
        'blocked',
      )
      await db.expectError(
      ...
      const chat = await db.rpc<{ id: string }>(
        'conversation_group_create',
        { human_ids: [alice.humanId, bob.humanId] },
        carol.as,
      )
      const aliceBefore = (await notificationsFor(db, alice)).filter(
        (n) => n.actor_human_id === bob.humanId,
      ).length
      ...
      expect(
        (await notificationsFor(db, carol)).filter((n) => n.actor_human_id === bob.humanId).length,
      ).toBe(carolBefore + 1)
      const queued = await db.rpc<Array<{ recipientHumanId: string; actorHumanId: string | null }>>(
      ...
      expect(
        queued.filter(
          (n) => n.recipientHumanId === alice.humanId && n.actorHumanId === bob.humanId,
        ),
      ).toEqual([])
    })
    ...
    it('direct surfaces stay closed: direct rooms, the old DM, and group search across the block', async () => {
      await db.expectError(
        db.rpc(
          'room_start',
          { context_type: 'direct', context_id: aliceBobDm, title: null },
          bob.as,
        ),
        'blocked',
      )
      ...
      await db.expectError(
        db.rpc('conversation_get', { conversation_id: aliceBobDm }, bob.as),
        'blocked',
      )
      const bobsList = await db.rpc<{ conversations: Array<{ id: string }> }>(
        'conversations_list',
        {},
        bob.as,
      )
      ...
      expect((await search(db, bob.as, 'Alice Crew')).groups.map((g) => g.groupId)).not.toContain(
        crew.groupId,
      )
      expect((await search(db, carol.as, 'Alice Crew')).groups.map((g) => g.groupId)).toContain(
        crew.groupId,
      )
    })
    ...
    it('replies by the blocked pair under a third party’s post are hidden from each other', async () => {
      ...
      const byAlice = await createPost(db, alice, {
        audience: 'world',
        text: 'alice replies',
        parentPostId: root.post.id,
      })
      const byBob = await createPost(db, bob, {
        audience: 'world',
        text: 'bob replies',
        parentPostId: root.post.id,
      })
      const repliesFor = async (as: RoleSpec) =>
        (
          await db.rpc<{ replies: Array<{ post: { id: string } }> }>(
            'post_replies',
  ...
  describe('authorization matrix: relationships and blocks per caller kind', () => {
    it('visitor denied; guest, claiming, friend, stranger and blocked see nothing of a pair; source and target see their own edges', async () => {
      ...
      await expect(
        db.asRole('visitor', (c) => c.query('select * from public.blocks')),
      ).rejects.toMatchObject({ code: PERMISSION_DENIED })
      for (const as of [
        guest.as,
        claiming.as,
      ...
      expect(await relationshipRowsAs(db, source.as, source, target)).toEqual([
        'ab:familiar_private',
        'ab:follow',
        'ba:friend_pending',
      ])
      expect(await relationshipRowsAs(db, target.as, source, target)).toEqual([
        'ab:follow',
        'ba:friend_pending',
      ])
      expect(
        (await rowsAs(db, source.as, 'select blocked_human_id from public.blocks')).map(
          (r) => r['blocked_human_id'],
        ),
      ).toEqual([blockedBySource.humanId])
      ...
      for (const rpc of ['friend_request_send', 'follow_set', 'block_set'] as const) {
        await db.expectError(
          db.rpc(rpc, { target_human_id: target.humanId }, 'visitor'),
          'not_authenticated',
        )
        await db.expectError(
          db.rpc(rpc, { target_human_id: target.humanId }, guest.as),
          'not_a_human',
        )
        await db.expectError(
          db.rpc(rpc, { target_human_id: target.humanId }, claiming.as),
          'not_a_human',
        )
        await db.expectError(
          db.rpc(rpc, { target_human_id: target.humanId }, pending.as),
          'not_a_human',
        )
      }
      await db.expectError(
        db.rpc('friend_request_send', { target_human_id: source.humanId }, blockedBySource.as),
        'blocked',
      )
      await db.expectError(
        db.rpc('follow_set', { target_human_id: source.humanId }, blockedBySource.as),
        'blocked',
      )
    })
  })
