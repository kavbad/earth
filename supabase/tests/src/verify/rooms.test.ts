...
interface Candidate {
  ...
  participants: Array<{ humanId: string | null; displayName: string; isGuest: boolean; relationToViewer: string | null }>
}

async function liveList(db: TestDb, scope: string, as: RoleSpec): Promise<Candidate[]> {
  const list = await db.rpc<{ candidates: Candidate[] }>('live_candidates', { scope, area_id: null }, as)
...
async function sync(db: TestDb, roomId: string, identity: string, event: string, at: string | null = null) {
  return db.rpc<{ applied: boolean; ignored: boolean; reason: string | null; transferredTo?: string | null }>(
    'room_participant_sync',
    { room_id: roomId, livekit_identity: identity, event, at },
    'service',
  )
}
...
describe('rooms invariants — adversarial verification', () => {
  ...
  describe('Live is a Room state', () => {
    ...
    it('a pending widening is not a Live: nobody outside the context sees the room until every publisher consented', async () => {
      ...
      expect(await roomRow(db, roomId)).toMatchObject({ visibility: 'group', pending_visibility: 'friends' })
      ...
      expect(await rowsAs(db, friend.as, 'select id from public.rooms where id = $1', [roomId])).toEqual([])
      expect(await rowsAs(db, friend.as, 'select id from public.room_participants where room_id = $1', [roomId])).toEqual([])
      // The member consenting flips it on every surface at once.
      const applied = RoomVisibilityChangeDtoSchema.parse(await db.rpc('room_consent', { room_id: roomId, level: 'friends' }, member.as))
      expect(applied).toMatchObject({ applied: true, visibility: 'friends', pendingVisibility: null })
    ...
    it('an ended room is gone from every scope and pointer however it ended (moderator, reconciler, sweep)', async () => {
      ...
      for (const id of [a, b, c]) await db.rpc('room_set_visibility', { room_id: id, visibility: 'world' }, host.as)
      ...
      expect((await db.rpc<{ roomsEnded: number }>('rooms_sweep', {}, 'service')).roomsEnded).toBeGreaterThanOrEqual(1)

      for (const id of [a, b, c]) {
        expect(await roomRow(db, id)).toMatchObject({ status: 'ended', active_participant_count: 0 })
        ...
        await db.expectError(db.rpc('room_join', { room_id: id, media_state: 'watching' }, friend.as), 'room_not_found')
        await db.expectError(db.rpc('room_join', { room_id: id, media_state: 'camera', consent_level: 'world' }, host.as), 'room_ended')
        // A former participant keeps the history, nobody else.
        expect((await getRoom(db, id, host.as)).status).toBe('ended')
      }
      expect(await scalar(db, 'count(*) from public.human_presence where active_room_id in ($1, $2, $3)', [a, b, c])).toBe('0')
    })
    ...
    it('invited and group rooms never surface in discovery, even for friends of every publisher', async () => {
      ...
      const directRoom = (await db.rpc<{ room: { id: string } }>('room_start', { context_type: 'direct', context_id: dm }, owner.as)).room.id
      ...
      expect(await rowsAs(db, friend.as, 'select id from public.rooms where id in ($1, $2)', [groupRoom, directRoom])).toEqual([])
  ...
  describe('naming follows the participants', () => {
    it('payloads track joins, upgrades, guests and leaves; watchers never surface outward', async () => {
      ...
      await db.rpc('room_set_media_state', { room_id: roomId, media_state: 'audio', consent_level: 'friends' }, viewer.as)
      ...
      expect(room.participants.map((p) => [p.isGuest, p.humanId === null, p.guestSessionId !== null])).toEqual([
        [false, false, false],
        [false, false, false],
        [true, true, true],
      ])
      ...
      expect((await getRoom(db, roomId, host.as)).participants.map((p) => p.displayName)).toEqual(['Nhost', 'Nviewer'])
      expect(await rowsAs(db, outsider.as, 'select human_id from public.room_participants where room_id = $1 and human_id = $2', [roomId, viewer.humanId])).toEqual([])
    ...
    it('waiting and invited seats are named only inside the room', async () => {
      ...
      await db.rpc('room_set_visibility', { room_id: roomId, visibility: 'world', join_policy: 'request' }, host.as)
      ...
      expect((await liveList(db, 'world', outsider.as)).find((c) => c.roomId === roomId)?.participantCount).toBe(1)
      expect((await getRoom(db, roomId, host.as)).participants.map((p) => [p.displayName, p.status])).toEqual([
        ['Whost', 'active'],
        ['Wrequester', 'waiting'],
      ])
      expect(await rowsAs(db, outsider.as, 'select id from public.room_participants where room_id = $1 and human_id = $2', [roomId, requester.humanId])).toEqual([])
      await db.rpc('room_end', { room_id: roomId }, host.as)
    })
  })
  ...
  describe('consent gating — no hidden audience inheritance', () => {
    it('admitting a waiting seat after a widening never puts it on camera beyond its consent', async () => {
      ...
      await befriend(db, host, friend)
      const roomId = (await startStandaloneRoom(db, host)).room.id
      await db.rpc('room_set_visibility', { room_id: roomId, visibility: 'friends', join_policy: 'request' }, host.as)
      const request = await joinRoom(db, roomId, friend, 'camera', 'friends')
      expect(request.myParticipant).toMatchObject({ status: 'waiting', mediaState: 'camera', audienceConsentLevel: 'friends' })
      ...
      await db.expectError(db.rpc('room_set_media_state', { room_id: roomId, media_state: 'camera' }, friend.as), 'consent_required')
      // Explicit consent, then camera: the seat was admitted, so no second request.
      await db.rpc('room_set_media_state', { room_id: roomId, media_state: 'camera', consent_level: 'world' }, friend.as)
      expect(await participantStatus(db, roomId, friend.humanId)).toMatchObject({ status: 'active', media_state: 'camera', consent: 'world' })
    ...
    it('the LiveKit reconciler never re-seats a stale row on camera beyond its consent', async () => {
      ...
      const left = RoomLeaveDtoSchema.parse(await db.rpc('room_leave', { room_id: roomId }, host.as))
      expect(left.transferredTo).toBe(cohost.humanId)
      const opened = RoomVisibilityChangeDtoSchema.parse(await db.rpc('room_set_visibility', { room_id: roomId, visibility: 'world' }, cohost.as))
      expect(opened.applied).toBe(true)
      ...
      const rejoined = await sync(db, roomId, `h:${host.humanId}`, 'participant_joined', secondsFromNow(1))
    ...
    it('a moderator opening up consents for nobody else — audio publishers included', async () => {
      ...
      expect(pending.pendingParticipantIds.sort()).toEqual([participantId(a, audio.humanId), participantId(c, camera.humanId)].sort())
      expect(await participantStatus(db, roomId, audio.humanId)).toMatchObject({ consent: 'group' })
      expect(await participantStatus(db, roomId, camera.humanId)).toMatchObject({ consent: 'group' })
      ...
      const half = RoomVisibilityChangeDtoSchema.parse(await db.rpc('room_consent', { room_id: roomId, level: 'friends' }, audio.as))
      expect(half).toMatchObject({ applied: false, pendingParticipantIds: [participantId(c, camera.humanId)] })
      // A re-join by the pending participant with a low consent changes nothing.
      await joinRoom(db, roomId, camera, 'camera', 'group')
      expect(await roomRow(db, roomId)).toMatchObject({ visibility: 'group', pending_visibility: 'friends' })
      const done = RoomVisibilityChangeDtoSchema.parse(await db.rpc('room_set_media_state', { room_id: roomId, media_state: 'watching' }, camera.as))
    ...
    it('consent is per Human and per level: a viewer with no or a lower consent cannot publish in a wider room', async () => {
      ...
      await befriend(db, host, friend)
      const roomId = (await startStandaloneRoom(db, host)).room.id
      await db.rpc('room_set_visibility', { room_id: roomId, visibility: 'world', join_policy: 'friends' }, host.as)
      await joinRoom(db, roomId, friend, 'watching')
      await db.expectError(db.rpc('room_set_media_state', { room_id: roomId, media_state: 'camera' }, friend.as), 'consent_required')
      await db.expectError(db.rpc('room_set_media_state', { room_id: roomId, media_state: 'audio', consent_level: 'city' }, friend.as), 'consent_required')
      await db.expectError(db.rpc('room_join', { room_id: roomId, media_state: 'camera', consent_level: 'city' }, friend.as), 'consent_required')
      expect((await grant(db, roomId, friend.as)).canPublish).toBe(false)
      await db.rpc('room_set_media_state', { room_id: roomId, media_state: 'camera', consent_level: 'world' }, friend.as)
      expect((await grant(db, roomId, friend.as)).canPublish).toBe(true)
      await db.rpc('room_end', { room_id: roomId }, host.as)
    ...
    it('the media grant never says canPublish for a Human whose recorded consent is below the room (defence in depth)', async () => {
      ...
      await db.sql.query(`update public.room_participants set audience_consent_level = 'friends' where room_id = $1 and human_id = $2`, [roomId, friend.humanId])
      expect((await grant(db, roomId, friend.as)).canPublish).toBe(false)
      await db.rpc('room_end', { room_id: roomId }, host.as)
    })
  })
  ...
  describe('moderator transfer never to Guests; the initiator outranks moderators', () => {
    it('transfer skips Guests and non-active Humans in every path; no Guest row can ever hold a moderator role', async () => {
      ...
      const invite = await createRoomInvite(db, roomId, host)
      const guest = await createGuest(db)
      const session = await createGuestSession(db, guest, invite.token, 'Sam', { mediaState: 'camera' })
      await joinRoom(db, roomId, watcher, 'watching')
      expect((await joinRoom(db, roomId, requester, 'camera', 'world')).myParticipant?.status).toBe('waiting')
      ...
      const dropped = await sync(db, roomId, `h:${watcher.humanId}`, 'participant_left', secondsFromNow(1))
      expect(dropped).toMatchObject({ applied: true, transferredTo: null })
      expect(await count(db, 'public.room_participants', "room_id = $1 and status = 'active' and role in ('initiator', 'moderator')", [roomId])).toBe(0)
      expect(await scalar(db, "role::text from public.room_participants where guest_session_id = $1 and status = 'active'", [session.guestSessionId])).toBe('participant')
      // Not even the database owner can give a Guest row a moderator role.
      expect(
        await sqlErrorCode(db.sql.query(`update public.room_participants set role = 'moderator' where guest_session_id = $1`, [session.guestSessionId])),
      ).toBe(CHECK_VIOLATION)
      // A Human coming back takes the room over (spec §61: Guests cannot own the room).
      await joinRoom(db, roomId, watcher, 'watching')
      expect(await participantStatus(db, roomId, watcher.humanId)).toMatchObject({ role: 'moderator', status: 'active' })
    ...
    it('a moderator cannot eject the initiator by blocking them: the lower rank leaves', async () => {
      ...
      expect(RoomLeaveDtoSchema.parse(await db.rpc('room_leave', { room_id: roomId }, host.as)).transferredTo).toBe(mod.humanId)
      await joinRoom(db, roomId, host, 'camera', 'friends')
      expect(await participantStatus(db, roomId, host.humanId)).toMatchObject({ role: 'initiator', status: 'active' })
      expect(await participantStatus(db, roomId, mod.humanId)).toMatchObject({ role: 'moderator', status: 'active' })
      ...
      await db.expectError(db.rpc('room_remove_participant', { room_id: roomId, participant_id: hostSeat }, mod.as), 'forbidden')
  ...
  describe('grace-period end', () => {
    it('is measured from the last active Human leaving and honours app_settings.room_grace_seconds', async () => {
      ...
      try {
        ...
        const absentSince = await scalar<Date>(db, 'humans_absent_since from public.rooms where id = $1', [roomId])
        expect(Math.abs(absentSince.getTime() - new Date(leftAt).getTime())).toBeLessThan(1000)
        expect((await rpcAt<{ roomsEnded: number }>(db, 'rooms_sweep', {}, 'service', secondsFromNow(25))).roomsEnded).toBe(0)
        expect((await roomRow(db, roomId)).status).toBe('active')
        expect((await rpcAt<{ roomsEnded: number }>(db, 'rooms_sweep', {}, 'service', secondsFromNow(31))).roomsEnded).toBe(1)
        ...
        expect(await count(db, 'public.guest_sessions', "room_id = $1 and expires_at <= now() + interval '11 minutes'", [roomId])).toBe(1)
      } finally {
    ...
    it('waiting and invited Humans do not keep a room alive; only active Humans reset the clock', async () => {
      ...
      const direct = (await db.rpc<{ room: { id: string } }>('room_start', { context_type: 'direct', context_id: dm }, host.as)).room.id
      await db.rpc('room_leave', { room_id: direct }, host.as)
      expect(await roomRow(db, direct)).toMatchObject({ active_human_count: 0, active_participant_count: 0 })
      expect((await db.rpc<{ roomsEnded: number }>('rooms_sweep', {}, 'service')).roomsEnded).toBe(1)
      ...
      expect((await joinRoom(db, roomId, requester, 'camera', 'world')).myParticipant?.status).toBe('waiting')
      expect((await rpcAt<{ roomsEnded: number }>(db, 'rooms_sweep', {}, 'service', secondsFromNow(60))).roomsEnded).toBe(0)
      // A friend coming back to watch resets the clock and takes the room over.
      await rpcAt(db, 'room_join', { room_id: roomId, media_state: 'watching', consent_level: 'invited' }, friend.as, secondsFromNow(90))
      expect(await scalar(db, 'humans_absent_since from public.rooms where id = $1', [roomId])).toBeNull()
      expect((await rpcAt<{ roomsEnded: number }>(db, 'rooms_sweep', {}, 'service', secondsFromNow(300))).roomsEnded).toBe(0)
      expect(await participantStatus(db, roomId, friend.humanId)).toMatchObject({ role: 'moderator' })
      await rpcAt(db, 'room_leave', { room_id: roomId }, friend.as, secondsFromNow(100))
      expect((await rpcAt<{ roomsEnded: number }>(db, 'rooms_sweep', {}, 'service', secondsFromNow(221))).roomsEnded).toBe(1)
      expect(await roomRow(db, roomId)).toMatchObject({ status: 'ended', ended_reason: 'no_humans' })
      expect(await participantStatus(db, roomId, requester.humanId)).toMatchObject({ status: 'left' })
    })

    it('the sweep and the reconciler are service-only for every other caller kind', async () => {
      const guest = await createGuest(db)
      const claiming = await createHuman(db, { handle: nextHandle('sweepclaim'), status: 'pending' })
      ...
      for (const as of ['visitor', guest.as, claiming.as, h.as] as const) {
        ...
        expect(await sqlErrorCode(db.rpc('room_participant_sync', { room_id: randomUUID(), livekit_identity: 'h:x', event: 'room_finished' }, as))).toBe(PERMISSION_DENIED)
      }
    })
  })
  ...
  describe('the token grant never exceeds the seat', () => {
    ...
    it('visitors, Guests of other rooms, claiming and unclaimed credentials, strangers, waiting and invited seats get no grant', async () => {
      await db.expectError(db.rpc('room_media_grant', { room_id: roomId }, 'visitor'), 'not_authenticated')
      await db.expectError(db.rpc('room_media_grant', { room_id: roomId }, (await createGuest(db)).as), 'not_in_room')
      const claiming = await createHuman(db, { handle: nextHandle('tokclaim'), status: 'pending' })
      await db.expectError(db.rpc('room_media_grant', { room_id: roomId }, claiming.as), 'not_a_human')
      await db.expectError(db.rpc('room_media_grant', { room_id: roomId }, (await createUnclaimed(db)).as), 'not_a_human')
      const stranger = await human(db, 'Tstranger')
      await db.expectError(db.rpc('room_media_grant', { room_id: roomId }, stranger.as), 'not_in_room')
      expect((await joinRoom(db, roomId, stranger, 'camera', 'world')).myParticipant?.status).toBe('waiting')
      await db.expectError(db.rpc('room_media_grant', { room_id: roomId }, stranger.as), 'not_in_room')
    ...
    it('canPublish and role follow the seat exactly: viewer → publisher → viewer, transfer, removal, suspension, end', async () => {
      ...
      expect(await grant(db, roomId, member.as)).toMatchObject({ role: 'viewer', canPublish: false, canSubscribe: true, identity: `h:${member.humanId}` })
      await db.rpc('room_set_media_state', { room_id: roomId, media_state: 'audio', consent_level: 'world' }, member.as)
      expect(await grant(db, roomId, member.as)).toMatchObject({ role: 'participant', canPublish: true })
      await db.rpc('room_set_media_state', { room_id: roomId, media_state: 'watching' }, member.as)
      expect(await grant(db, roomId, member.as)).toMatchObject({ role: 'viewer', canPublish: false })
      // Transfer shows in the grant; the initiator coming back is the initiator again.
      expect(RoomLeaveDtoSchema.parse(await db.rpc('room_leave', { room_id: roomId }, host.as)).transferredTo).toBe(member.humanId)
      expect(await grant(db, roomId, member.as)).toMatchObject({ role: 'moderator', canPublish: false })
      ...
      expect(await grant(db, roomId, host.as)).toMatchObject({ role: 'initiator', canPublish: true })
      // A Guest is always participant/viewer with a g: identity.
      const invite = await createRoomInvite(db, roomId, host)
      const guest = await createGuest(db)
      const session = await createGuestSession(db, guest, invite.token, 'Sam', { mediaState: 'watching' })
      expect(await grant(db, roomId, guest.as)).toMatchObject({ role: 'viewer', canPublish: false, identity: `g:${session.guestSessionId}`, name: 'Sam' })
      await db.rpc('room_set_media_state', { room_id: roomId, media_state: 'camera' }, guest.as)
      expect(await grant(db, roomId, guest.as)).toMatchObject({ role: 'participant', canPublish: true })
      ...
      await db.expectError(db.rpc('room_media_grant', { room_id: roomId }, member.as), 'not_in_room')
      ...
      await db.sql.query(`update public.humans set status = 'suspended' where id = $1`, [suspended.humanId])
      await db.expectError(db.rpc('room_media_grant', { room_id: roomId }, suspended.as), 'human_not_active')
      await db.sql.query(`update public.humans set status = 'active' where id = $1`, [suspended.humanId])
  ...
  describe('Guests cannot expand, invite, moderate, discover or DM', () => {
    ...
    it('every moderator, invite and discovery affordance refuses the Guest', async () => {
      ...
      const calls: Array<[string, Record<string, unknown>, string]> = [
        ...
        ['room_remove_participant', { room_id: roomId, participant_id: cohostSeat }, 'guest_not_allowed'],
        ...
        ['room_invite_join', { token, media_state: 'camera', consent_level: 'world' }, 'not_a_human'],
      ...
      expect(await roomRow(db, roomId)).toMatchObject({ visibility: 'friends', join_policy: 'friends', status: 'active' })
    ...
    it('a Guest cannot DM, friend, follow or read conversations of the Humans they met', async () => {
      const calls: Array<[string, Record<string, unknown>]> = [
        ...
        ['message_send', { conversation_id: dm, client_id: randomUUID(), type: 'text', text: 'hi from a guest' }],
      ...
      for (const table of ['conversations', 'conversation_members', 'messages', 'relationships', 'blocks']) {
        const rows = await rowsAs(db, guest.as, `select * from public.${table}`).catch((error: unknown) => {
    ...
    it('a Guest exists in exactly one room: no other room is readable, joinable or grantable', async () => {
      ...
      await db.expectError(db.rpc('room_join', { room_id: other, media_state: 'audio' }, guest.as), 'guest_not_allowed')
      await db.expectError(db.rpc('room_media_grant', { room_id: other }, guest.as), 'not_in_room')
      await db.expectError(db.rpc('room_set_media_state', { room_id: other, media_state: 'camera' }, guest.as), 'not_in_room')
    ...
    it('"guests disabled" is temporary: re-enabling lets the same Guest credential come back; a moderator removal does not', async () => {
      ...
      await db.expectError(db.rpc('room_join', { room_id: roomId, media_state: 'audio' }, guest.as), 'guests_disabled')
      ...
      const back = RoomDtoSchema.parse(await db.rpc('room_join', { room_id: roomId, media_state: 'audio' }, guest.as).catch(async () => {
        await createGuestSession(db, guest, token, 'Sam')
        return db.rpc('room_get', { room_id: roomId }, guest.as)
      }))
      expect(back.myParticipant).toMatchObject({ status: 'active', isGuest: true })
      await createGuestSession(db, kicked, token, 'Pat')
      expect((await getRoom(db, roomId, host.as)).participants.filter((p) => p.isGuest)).toHaveLength(2)
      // A removal by a moderator is final for that credential.
      const patSeat = (await getRoom(db, roomId, host.as)).participants.find((p) => p.displayName === 'Pat')?.id
      await db.rpc('room_remove_participant', { room_id: roomId, participant_id: patSeat }, host.as)
      await db.expectError(db.rpc('guest_session_create', { token, display_name: 'Pat' }, kicked.as), 'blocked')
      await db.expectError(db.rpc('room_join', { room_id: roomId, media_state: 'audio' }, kicked.as), 'guest_not_allowed')
    })
    ...
    it('GUEST_ROOMS_ENABLED is a kill switch: Guests already inside stop joining and minting grants', async () => {
      ...
      try {
        await db.expectError(db.rpc('guest_session_create', { token, display_name: 'Sam' }, (await createGuest(db)).as), 'feature_disabled')
        await db.expectError(db.rpc('room_media_grant', { room_id: roomId }, inside.as), 'feature_disabled')
        await db.rpc('room_leave', { room_id: roomId }, inside.as)
        await db.expectError(db.rpc('room_join', { room_id: roomId, media_state: 'audio' }, inside.as), 'feature_disabled')
        expect((await sync(db, roomId, `g:${await scalar<string>(db, 'id from public.guest_sessions where auth_user_id = $1', [inside.userId])}`, 'participant_joined', secondsFromNow(1))).applied).toBe(false)
      ...
      expect(RoomDtoSchema.parse(await db.rpc('room_join', { room_id: roomId, media_state: 'audio' }, inside.as)).myParticipant?.status).toBe('active')
      expect((await grant(db, roomId, inside.as)).canPublish).toBe(true)
    })
  })
  ...
  describe('join policy is enforced on the first publish, not only on the first entry', () => {
    it('watch first, then camera: invited_only still refuses; request queues; admission and links persist; toggling never re-asks', async () => {
      ...
      const roomId = (await startStandaloneRoom(db, host)).room.id
      await db.rpc('room_set_visibility', { room_id: roomId, visibility: 'world', join_policy: 'invited_only' }, host.as)

      // A stranger may watch a world room; watching is not an invitation to publish.
      expect((await joinRoom(db, roomId, stranger, 'watching')).myParticipant?.status).toBe('active')
      await db.expectError(db.rpc('room_join', { room_id: roomId, media_state: 'camera', consent_level: 'world' }, stranger.as), 'join_not_allowed')
      await db.expectError(db.rpc('room_set_media_state', { room_id: roomId, media_state: 'camera', consent_level: 'world' }, stranger.as), 'join_not_allowed')
      await db.expectError(db.rpc('room_set_media_state', { room_id: roomId, media_state: 'audio', consent_level: 'world' }, stranger.as), 'join_not_allowed')
      expect(await participantStatus(db, roomId, stranger.humanId)).toMatchObject({ status: 'active', media_state: 'watching', role: 'viewer' })
      ...
      await db.rpc('room_set_media_state', { room_id: roomId, media_state: 'camera', consent_level: 'world' }, stranger.as)
      expect(await participantStatus(db, roomId, stranger.humanId)).toMatchObject({ status: 'waiting' })
      await db.expectError(db.rpc('room_media_grant', { room_id: roomId }, stranger.as), 'not_in_room')
      expect(await publishersSeenBy(db, roomId, 'visitor')).toEqual(['Phost'])
      const seat = (await getRoom(db, roomId, host.as)).participants.find((p) => p.humanId === stranger.humanId)?.id
      await db.rpc('room_admit', { room_id: roomId, participant_id: seat }, host.as)
      expect(await participantStatus(db, roomId, stranger.humanId)).toMatchObject({ status: 'active', media_state: 'camera' })
      ...
      await db.rpc('room_set_join_policy', { room_id: roomId, join_policy: 'invited_only' }, host.as)
      await db.rpc('room_set_media_state', { room_id: roomId, media_state: 'watching' }, stranger.as)
      ...
      const invite = await createRoomInvite(db, roomId, host)
      expect(RoomDtoSchema.parse(await db.rpc('room_invite_join', { token: invite.token, media_state: 'watching' }, linker.as)).myParticipant?.status).toBe('active')
      await db.rpc('room_set_media_state', { room_id: roomId, media_state: 'camera', consent_level: 'world' }, linker.as)
      ...
      expect(RoomDtoSchema.parse(await db.rpc('room_invite_join', { token: invite.token, media_state: 'camera', consent_level: 'world' }, returning.as)).myParticipant?.status).toBe('active')
      await db.rpc('room_leave', { room_id: roomId }, returning.as)
      expect((await joinRoom(db, roomId, returning, 'camera', 'world')).myParticipant?.status).toBe('active')
      ...
      await db.expectError(db.rpc('room_join', { room_id: roomId, media_state: 'camera', consent_level: 'world' }, watcher.as), 'join_not_allowed')
      await db.rpc('room_end', { room_id: roomId }, host.as)
    })
    ...
    it('a group member or friend upgrading from a watching seat passes the policy on their own merits', async () => {
      ...
      await befriend(db, owner, friend)
      const roomId = (await startGroupRoom(db, owner, group)).room.id
      await db.rpc('room_set_visibility', { room_id: roomId, visibility: 'friends', join_policy: 'group' }, owner.as)
      await joinRoom(db, roomId, member, 'watching')
      await joinRoom(db, roomId, friend, 'watching')
      await db.rpc('room_set_media_state', { room_id: roomId, media_state: 'camera', consent_level: 'friends' }, member.as)
      expect(await participantStatus(db, roomId, member.humanId)).toMatchObject({ media_state: 'camera', status: 'active' })
      await db.expectError(db.rpc('room_set_media_state', { room_id: roomId, media_state: 'camera', consent_level: 'friends' }, friend.as), 'join_not_allowed')
      await db.rpc('room_set_join_policy', { room_id: roomId, join_policy: 'friends' }, owner.as)
      await db.rpc('room_set_media_state', { room_id: roomId, media_state: 'camera', consent_level: 'friends' }, friend.as)
      expect(await participantStatus(db, roomId, friend.humanId)).toMatchObject({ media_state: 'camera', status: 'active' })
      await db.rpc('room_end', { room_id: roomId }, owner.as)
    })
  })
  ...
  describe('join policy × visibility pairs', () => {
    ...
    beforeAll(async () => {
      ...
      neighborhood = await createArea(db, { name: 'Pair Hood', slug: 'pair-hood', type: 'neighborhood', parentAreaId: city })
    })

    /** What the database must answer for `room_set_visibility(v, p)` on a solo room of `contextType`. */
    function expectedPair(contextType: 'group' | 'standalone', v: RoomVisibility, p: RoomJoinPolicy): string | null {
      if ((v === 'invited' && contextType === 'group') || (v === 'group' && contextType !== 'group')) return 'visibility_not_allowed'
      ...
      it(`room_set_visibility on a ${contextType} room accepts exactly the pairs the domain offers`, async () => {
        ...
        for (const v of ROOM_VISIBILITY) {
          for (const p of ROOM_JOIN_POLICY) {
            ...
            const code = await errorCode(db.rpc('room_set_visibility', { room_id: roomId, visibility: v, join_policy: p }, host.as))
            expect(code, `${contextType} ${v}/${p}`).toBe(expected)
            if (expected === null) {
              expect(await roomRow(db, roomId), `${contextType} ${v}/${p} applied`).toMatchObject({ visibility: v, join_policy: p, pending_visibility: null })
            }
          }
        }
        await db.rpc('room_end', { room_id: roomId }, host.as)
      ...
      it(`room_set_join_policy on a ${contextType} room accepts exactly allowedJoinPoliciesFor(visibility, contextType)`, async () => {
        ...
        const roomId =
          contextType === 'group'
            ...
            : (await startStandaloneRoom(db, host)).room.id
        for (const v of ROOM_VISIBILITY) {
          if (expectedPair(contextType, v, allowedJoinPoliciesFor(v, contextType)[0]!) !== null) continue
          await db.rpc('room_set_visibility', { room_id: roomId, visibility: v }, host.as)
          for (const p of ROOM_JOIN_POLICY) {
            const code = await errorCode(db.rpc('room_set_join_policy', { room_id: roomId, join_policy: p }, host.as))
            expect(code, `${contextType} ${v} → ${p}`).toBe(isJoinPolicyAllowedFor(v, p, contextType) ? null : 'invalid_input')
          }
    ...
    it('an invite override is a moderator affordance and must be offered for the visibility (or be the link policy)', async () => {
      ...
      await db.expectError(db.rpc('room_invite_create', { room_id: roomId, join_policy_override: 'anyone_with_link' }, friend.as), 'not_a_moderator')
      await db.expectError(db.rpc('room_invite_create', { room_id: roomId, join_policy_override: 'group' }, host.as), 'invalid_input')
      await db.expectError(db.rpc('room_invite_create', { room_id: roomId, join_policy_override: 'anyone' }, host.as), 'invalid_input')
      await db.rpc('room_invite_create', { room_id: roomId, join_policy_override: 'anyone_with_link' }, host.as)
      await db.rpc('room_invite_create', { room_id: roomId, join_policy_override: 'request' }, host.as)
  ...
  describe('feature flags', () => {
    ...
    it('FRIENDS_LIVE_EXPANSION_ENABLED gates standalone rooms and every widening to friends or extended; narrowing never', async () => {
      ...
      try {
        await db.expectError(db.rpc('room_start', { context_type: 'standalone' }, owner.as), 'feature_disabled')
        expect(RoomVisibilityChangeDtoSchema.parse(await db.rpc('room_set_visibility', { room_id: roomId, visibility: 'group' }, owner.as)).applied).toBe(true)
        await db.expectError(db.rpc('room_set_visibility', { room_id: roomId, visibility: 'friends' }, owner.as), 'feature_disabled')
        await db.expectError(db.rpc('room_set_visibility', { room_id: roomId, visibility: 'extended' }, owner.as), 'feature_disabled')
        await db.expectError(db.rpc('room_set_visibility', { room_id: roomId, visibility: 'world' }, owner.as), 'feature_disabled')
      } finally {
    ...
    it('a widening that was pending when its flag went off never applies', async () => {
      ...
      const roomId = (await startGroupRoom(db, owner, group)).room.id
      await joinRoom(db, roomId, member, 'camera', 'group')
      expect(RoomVisibilityChangeDtoSchema.parse(await db.rpc('room_set_visibility', { room_id: roomId, visibility: 'friends' }, owner.as)).applied).toBe(false)
      await setFlag(db, 'FRIENDS_LIVE_EXPANSION_ENABLED', false)
      try {
        const consent = RoomVisibilityChangeDtoSchema.parse(await db.rpc('room_consent', { room_id: roomId, level: 'friends' }, member.as))
      ...
      expect(await roomRow(db, roomId)).toMatchObject({ visibility: 'group' })
      expect(RoomVisibilityChangeDtoSchema.parse(await db.rpc('room_set_visibility', { room_id: roomId, visibility: 'friends' }, owner.as)).applied).toBe(true)
      await db.rpc('room_end', { room_id: roomId }, owner.as)
    })
    ...
    it('WORLD_LIVE_EXPANSION_ENABLED and PUBLIC_LIVE_ENABLED gate neighborhood/city/world, and PUBLIC_LIVE_ENABLED gates visitors', async () => {
      ...
      for (const flag of ['WORLD_LIVE_EXPANSION_ENABLED', 'PUBLIC_LIVE_ENABLED'] as const) {
        ...
        try {
          for (const v of ['neighborhood', 'city', 'world'] as const) {
            await db.expectError(db.rpc('room_set_visibility', { room_id: roomId, visibility: v }, host.as), 'feature_disabled')
          }
          expect(RoomVisibilityChangeDtoSchema.parse(await db.rpc('room_set_visibility', { room_id: roomId, visibility: 'extended' }, host.as)).applied).toBe(true)
        } finally {
          await setFlag(db, flag, true)
        }
      }
      expect(RoomVisibilityChangeDtoSchema.parse(await db.rpc('room_set_visibility', { room_id: roomId, visibility: 'world' }, host.as)).applied).toBe(true)
      ...
      try {
        ...
        expect(await rowsAs(db, 'visitor', 'select id from public.rooms where id = $1', [roomId])).toEqual([])
  ...
  describe('blocks and group membership hold inside live rooms', () => {
    it('the reconciler never re-seats a Human next to someone who blocked them since they left', async () => {
      ...
      const rejoin = await sync(db, roomId, `h:${friend.humanId}`, 'participant_joined', secondsFromNow(1))
      ...
      await db.expectError(db.rpc('room_get', { room_id: roomId }, friend.as), 'room_not_found')
      await db.expectError(db.rpc('room_media_grant', { room_id: roomId }, friend.as), 'not_in_room')
      await db.rpc('room_end', { room_id: roomId }, host.as)
    })
    ...
    it('leaving or being removed from the group ends the seat in the group room and every former-seat privilege', async () => {
      ...
      await db.rpc('group_member_remove', { group_id: group.groupId, human_id: removed.humanId }, owner.as)
      ...
      for (const gone of [removed, leaver]) {
        ...
        await db.expectError(db.rpc('room_media_grant', { room_id: roomId }, gone.as), 'not_in_room')
        await db.expectError(db.rpc('room_get', { room_id: roomId }, gone.as), 'room_not_found')
        await db.expectError(db.rpc('room_join', { room_id: roomId, media_state: 'watching' }, gone.as), 'room_not_found')
        await db.expectError(db.rpc('room_start', { context_type: 'group', context_id: group.groupId }, gone.as), 'not_a_member')
        expect(await rowsAs(db, gone.as, 'select id from public.rooms where id = $1', [roomId])).toEqual([])
      }
      expect((await getRoom(db, roomId, owner.as)).participants.map((p) => p.displayName)).toEqual(['Gowner3'])
      expect(await roomRow(db, roomId)).toMatchObject({ active_participant_count: 1, active_human_count: 1 })
      // Re-added, the seat can be taken again.
      await addMember(db, group, leaver)
      expect((await joinRoom(db, roomId, leaver, 'camera', 'group')).myParticipant?.status).toBe('active')
      await db.rpc('room_end', { room_id: roomId }, owner.as)
    })
  })
})
