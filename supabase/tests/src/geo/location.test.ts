...
describe('location sharing (DB_API §5)', () => {
  ...
  describe('location_share_create', () => {
    it('is for active Humans only', async () => {
      ...
      await db.expectError(db.rpc('location_share_create', args, (await createUnclaimed(db)).as), 'not_a_human')
    ...
    it('rejects a duration over 24 hours (25h → invalid_input) and non-positive durations', async () => {
      await db.expectError(
        db.rpc('location_share_create', shareArgs({ audienceId: bob.humanId, durationSeconds: 25 * HOUR }), alice.as),
        'invalid_input',
      )
      await db.expectError(
        db.rpc('location_share_create', shareArgs({ audienceId: bob.humanId, durationSeconds: 0 }), alice.as),
        'invalid_input',
      )
      await db.expectError(
        db.rpc('location_share_create', shareArgs({ audienceId: bob.humanId, durationSeconds: -60 }), alice.as),
        'invalid_input',
      )
      await db.expectError(
        db.rpc('location_share_create', { ...shareArgs({ audienceId: bob.humanId }), duration_seconds: null }, alice.as),
        'invalid_input',
      )
      // Exactly 24 hours is the maximum and is accepted.
      const day = await createShare(db, alice, { audienceId: bob.humanId, durationSeconds: 24 * HOUR })
      expect(new Date(day.expiresAt).getTime() - new Date(day.createdAt).getTime()).toBe(24 * HOUR * 1000)
    ...
    it('validates the position and the audience arguments', async () => {
      await db.expectError(
        db.rpc('location_share_create', shareArgs({ audienceId: bob.humanId, position: { lat: 91, lng: 0 } }), alice.as),
        'invalid_input',
      )
      await db.expectError(
        db.rpc('location_share_create', { ...shareArgs({ audienceId: bob.humanId }), lat: null }, alice.as),
        'invalid_input',
      )
      await db.expectError(
        db.rpc('location_share_create', { ...shareArgs({ audienceId: bob.humanId }), audience_id: null }, alice.as),
        'invalid_input',
      )
      await db.expectError(
        db.rpc('location_share_create', { ...shareArgs({ audienceId: bob.humanId }), precision: null }, alice.as),
        'invalid_input',
      )
      await db.expectError(db.rpc('location_share_create', shareArgs({ audienceId: alice.humanId }), alice.as), 'invalid_input')
    })

    it('a friend share requires friendship (not blocked, other Human active)', async () => {
      await db.expectError(db.rpc('location_share_create', shareArgs({ audienceId: carol.humanId }), alice.as), 'forbidden')
      await db.expectError(db.rpc('location_share_create', shareArgs({ audienceId: NIL_UUID }), alice.as), 'not_visible')
      const pending = await createHuman(db, { handle: 'pendingfriend', status: 'pending' })
      await db.expectError(db.rpc('location_share_create', shareArgs({ audienceId: pending.humanId }), alice.as), 'not_visible')
      const blocker = await human(db, 'Blocker')
      await befriend(db, alice, blocker)
      await block(db, blocker, alice)
      await db.expectError(db.rpc('location_share_create', shareArgs({ audienceId: blocker.humanId }), alice.as), 'blocked')
    })

    it('a group share requires active membership of an active group', async () => {
      const group = await createGroup(db, carol, 'Crew')
      await db.expectError(
        db.rpc('location_share_create', shareArgs({ audienceType: 'group', audienceId: group.groupId }), alice.as),
        'not_a_member',
      )
      await db.expectError(
        db.rpc('location_share_create', shareArgs({ audienceType: 'group', audienceId: NIL_UUID }), alice.as),
        'group_not_found',
      )
      await addMember(db, group, alice)
      const share = await createShare(db, alice, { audienceType: 'group', audienceId: group.groupId, precision: 'approximate' })
      expect(share).toMatchObject({ humanId: alice.humanId, audienceType: 'group', audienceId: group.groupId, precision: 'approximate' })
    ...
    it('a temporary-context share requires being an active participant of a live Room', async () => {
      await db.expectError(
        db.rpc('location_share_create', shareArgs({ audienceType: 'temporary_context', audienceId: NIL_UUID }), alice.as),
        'room_not_found',
      )
      const started = await startStandaloneRoom(db, carol)
      await db.expectError(
        db.rpc('location_share_create', shareArgs({ audienceType: 'temporary_context', audienceId: started.room.id }), alice.as),
        'not_in_room',
      )
      await db.rpc('room_end', { room_id: started.room.id, reason: null }, carol.as)
      await db.expectError(
        db.rpc('location_share_create', shareArgs({ audienceType: 'temporary_context', audienceId: started.room.id }), carol.as),
        'room_ended',
      )
    })

    it('returns a LocationShareDto, stores the initial position and replaces a live share to the same audience', async () => {
      const first = await createShare(db, alice, { audienceId: bob.humanId, precision: 'precise', durationSeconds: HOUR })
      ...
      expect(new Date(first.expiresAt).getTime() - new Date(first.createdAt).getTime()).toBe(HOUR * 1000)
      expect(await storedPosition(db, first.id)).toEqual({ lat: POINTS.northBeach.lat, lng: POINTS.northBeach.lng, cityAreaId: sf })
      ...
      expect(await count(db, 'public.location_shares', 'human_id = $1 and audience_id = $2 and revoked_at is null', [alice.humanId, bob.humanId])).toBe(1)
    ...
    it('is rate limited (30 per hour)', async () => {
      ...
      await db.expectError(db.rpc('location_share_create', shareArgs({ audienceId: grace.humanId }), frank.as), 'rate_limited')
    })
  })
  ...
  describe('precision degradation (spec §74)', () => {
    ...
    for (const precision of ['precise', 'approximate', 'city'] as const) {
      it(`a ${precision} share is visible to the friend at ${precision} precision`, async () => {
        const share = await createShare(db, alice, { audienceId: bob.humanId, precision, position: device })
        ...
        expect(await storedPosition(db, share.id)).toEqual({ lat: expected.lat, lng: expected.lng, cityAreaId: sf })
        await db.rpc('location_share_revoke', { share_id: share.id }, alice.as)
      })
    }
    ...
    it('snaps approximate shares to a 0.01° grid and city shares to the city centroid, whatever is stored', async () => {
      ...
      expect(expectedPosition('approximate', { lat: 37.7596, lng: -122.427 }, null)).toEqual({ lat: 37.76, lng: -122.43 })
      const share = await createShare(db, alice, { audienceId: bob.humanId, precision: 'approximate', position: { lat: 37.7596, lng: -122.427 } })
      ...
      await db.sql.query("update public.location_shares set precision = 'city' where id = $1", [share.id])
      const [asCity] = await visibleShares(db, bob.as)
      expect(asCity).toMatchObject({ lat: SF_CENTROID.lat, lng: SF_CENTROID.lng, precision: 'city' })
      await db.rpc('location_share_revoke', { share_id: share.id }, alice.as)
    })

    it('a city share outside every known city falls back to a 0.1° cell', async () => {
      const share = await createShare(db, alice, { audienceId: bob.humanId, precision: 'city', position: POINTS.ocean })
      expect(await storedPosition(db, share.id)).toEqual({ lat: snapTo(POINTS.ocean.lat, 1), lng: snapTo(POINTS.ocean.lng, 1), cityAreaId: null })
      const [visible] = await visibleShares(db, bob.as)
    ...
    it('location_share_update moves the position (owner only) with the same degradation', async () => {
      const share = await createShare(db, alice, { audienceId: bob.humanId, precision: 'approximate', position: POINTS.northBeach })
      const updated = LocationShareDtoSchema.parse(
        await db.rpc('location_share_update', { share_id: share.id, lat: POINTS.mission.lat, lng: POINTS.mission.lng }, alice.as),
      )
      expect(updated.id).toBe(share.id)
      const expected = expectedPosition('approximate', POINTS.mission, SF_CENTROID)
      expect(await storedPosition(db, share.id)).toEqual({ lat: expected.lat, lng: expected.lng, cityAreaId: sf })
      expect(await count(db, 'public.location_share_positions', 'share_id = $1', [share.id])).toBe(1)
      const [visible] = await visibleShares(db, bob.as)
      expect(visible).toMatchObject({ lat: expected.lat, lng: expected.lng })

      await db.expectError(db.rpc('location_share_update', { share_id: share.id, lat: 1, lng: 2 }, bob.as), 'not_visible')
      await db.expectError(db.rpc('location_share_update', { share_id: share.id, lat: 1, lng: 2 }, carol.as), 'not_visible')
      await db.expectError(db.rpc('location_share_update', { share_id: NIL_UUID, lat: 1, lng: 2 }, alice.as), 'not_visible')
      await db.expectError(db.rpc('location_share_update', { share_id: share.id, lat: 100, lng: 2 }, alice.as), 'invalid_input')
      await db.expectError(db.rpc('location_share_update', { share_id: share.id, lat: 1, lng: 2 }, 'visitor'), 'not_authenticated')
      await db.expectError(db.rpc('location_share_update', { share_id: share.id, lat: 1, lng: 2 }, guest), 'not_a_human')

      await db.rpc('location_share_revoke', { share_id: share.id }, alice.as)
      await db.expectError(db.rpc('location_share_update', { share_id: share.id, lat: 1, lng: 2 }, alice.as), 'invalid_input')
      const expired = await createShare(db, alice, { audienceId: bob.humanId, durationSeconds: 60 })
      await db.expectError(
        rpcAt(db, 'location_share_update', { share_id: expired.id, lat: 1, lng: 2 }, alice.as, new Date(Date.now() + 5 * 60 * 1000).toISOString()),
        'invalid_input',
      )
  ...
  describe('who sees a friend share', () => {
    ...
    it('a revoked share disappears and loses its position; revoking is idempotent and owner-only', async () => {
      ...
      await db.expectError(db.rpc('location_share_revoke', { share_id: share.id }, bob.as), 'not_visible')
      await db.expectError(db.rpc('location_share_revoke', { share_id: NIL_UUID }, alice.as), 'not_visible')
      await db.expectError(db.rpc('location_share_revoke', { share_id: share.id }, 'visitor'), 'not_authenticated')
      const revoked = LocationShareDtoSchema.parse(await db.rpc('location_share_revoke', { share_id: share.id }, alice.as))
      expect(revoked.revokedAt).not.toBeNull()
      expect(await visibleShares(db, bob.as)).toEqual([])
      expect(await storedPosition(db, share.id)).toBeNull()
      const again = LocationShareDtoSchema.parse(await db.rpc('location_share_revoke', { share_id: share.id }, alice.as))
    ...
    it('an expired share is invisible, and rooms_sweep revokes it and drops its position', async () => {
      ...
      const swept = await rpcAt<{ locationSharesRevoked: number }>(db, 'rooms_sweep', {}, 'service', later)
    ...
    it('a block in either direction hides the share; block_set also revokes shares between the two', async () => {
      ...
      expect(await visibleShares(db, bob.as)).toEqual([])
      await db.sql.query('delete from public.blocks where blocker_human_id = $1 and blocked_human_id = $2', [bob.humanId, alice.humanId])
      expect(await visibleShares(db, bob.as)).toHaveLength(1)
      await block(db, alice, bob)
      expect(await visibleShares(db, bob.as)).toEqual([])
      await db.sql.query('delete from public.blocks where blocker_human_id = $1 and blocked_human_id = $2', [alice.humanId, bob.humanId])
      expect(await visibleShares(db, bob.as)).toHaveLength(1)
    ...
    it('ending the friendship or deactivating the sharer hides the share', async () => {
      ...
      await befriend(db, alice, bob)
      expect(await visibleShares(db, bob.as)).toHaveLength(1)
      await db.sql.query("update public.humans set status = 'suspended' where id = $1", [alice.humanId])
      expect(await visibleShares(db, bob.as)).toEqual([])
      await db.sql.query("update public.humans set status = 'active' where id = $1", [alice.humanId])
      await db.rpc('location_share_revoke', { share_id: share.id }, alice.as)
    })
  })
  ...
  describe('who sees a group share', () => {
    it('active members of the group only, while the sharer is a member', async () => {
      ...
      const share = await createShare(db, owner, { audienceType: 'group', audienceId: group.groupId, precision: 'approximate' })
      const expected = expectedPosition('approximate', POINTS.northBeach, SF_CENTROID)
      const [visible] = await visibleShares(db, member.as)
      expect(visible).toMatchObject({ shareId: share.id, humanId: owner.humanId, audienceType: 'group', audienceId: group.groupId, ...expected })
      ...
      await db.sql.query("update public.group_members set status = 'left', left_at = now() where group_id = $1 and human_id = $2", [
        group.groupId,
        owner.humanId,
      ])
      expect(await visibleShares(db, outsider.as)).toEqual([])
  ...
  describe('who sees a temporary-context (Room) share', () => {
    it('active Human participants of the live Room only', async () => {
      ...
      const share = await createShare(db, alice, { audienceType: 'temporary_context', audienceId: roomId, precision: 'precise' })
      expect((await visibleShares(db, bob.as)).map((s) => s.shareId)).toEqual([share.id])
      expect(await visibleShares(db, carol.as)).toEqual([])
  ...
  describe('several shares at once', () => {
    it('lists every share that reaches the viewer, newest expiry first, each degraded on its own', async () => {
      ...
      const fromEve = await createShare(db, eve, { audienceType: 'group', audienceId: group.groupId, precision: 'city', durationSeconds: 3 * HOUR, position: POINTS.mission })
      const fromBob = await createShare(db, bob, { audienceId: finn.humanId, precision: 'precise', durationSeconds: 2 * HOUR, position: POINTS.goldenGatePark })
      const shares = await visibleShares(db, finn.as)
      expect(shares.map((s) => s.shareId)).toEqual([fromEve.id, fromBob.id])
      expect(shares[0]).toMatchObject({ humanId: eve.humanId, precision: 'city', lat: SF_CENTROID.lat, lng: SF_CENTROID.lng })
      expect(shares[1]).toMatchObject({ humanId: bob.humanId, precision: 'precise', lat: POINTS.goldenGatePark.lat, lng: POINTS.goldenGatePark.lng })
