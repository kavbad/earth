...
describe('room lifecycle (spec §57, §59, §61–§62; DB_API §3; ARCHITECTURE §10)', () => {
  ...
  describe('room_start', () => {
    it('creates a group room with the group defaults and the initiator on camera (RoomStartDto)', async () => {
      ...
      expect(
        await count(
          db,
          'public.messages',
          "conversation_id = $1 and type = 'system' and text like '%started a video'",
          [group.conversationId],
        ),
      ).toBe(1)
    ...
    it('group room start is idempotent: the second caller gets the existing room as a watching participant', async () => {
      ...
      expect(
        await count(db, 'public.rooms', "context_id = $1 and status = 'active'", [group.groupId]),
      ).toBe(1)
    })
    ...
    it('direct room invites both members and sets the conversation pointer', async () => {
      ...
      await db.expectError(
        db.rpc('room_get', { room_id: started.room.id }, outsider.as),
        'room_not_found',
      )
      await db.expectError(
    ...
    it('creating rooms is rate limited (20/h)', async () => {
      ...
      await db.expectError(
        db.rpc('room_start', { context_type: 'standalone' }, busy.as),
        'rate_limited',
      )
    })
  })
  ...
  describe('room_join consent gate', () => {
    it('joining on camera needs consent at least as wide as the room; watching needs none', async () => {
      ...
      await db.rpc(
        'room_set_visibility',
        { room_id: started.room.id, visibility: 'friends' },
        owner.as,
      )
      await db.expectError(
        db.rpc(
          'room_join',
          { room_id: started.room.id, media_state: 'camera', consent_level: 'group' },
          member.as,
        ),
        'consent_required',
      )
      await db.expectError(
        db.rpc(
          'room_join',
          { room_id: started.room.id, media_state: 'audio', consent_level: 'invited' },
          member.as,
        ),
        'consent_required',
      )
      const watching = await joinRoom(db, started.room.id, member, 'watching', 'invited')
      expect(watching.myParticipant).toMatchObject({
        mediaState: 'watching',
        role: 'viewer',
        status: 'active',
      })
    ...
    it('room_set_media_state applies the same gate and downgrades to viewer', async () => {
      ...
      await db.expectError(
        db.rpc(
          ...
          { room_id: started.room.id, media_state: 'camera' },
          member.as,
        ),
        'consent_required',
      )
      ...
      await db.rpc(
        'room_set_media_state',
        { room_id: started.room.id, media_state: 'watching' },
        member.as,
      )
      expect(await participantStatus(db, started.room.id, member.humanId)).toMatchObject({
  ...
  describe('join policies (mirror of canJoinWithMedia)', () => {
    ...
    it('friends policy admits friends of publishers, friends_of_friends one hop further, request creates waiting', async () => {
      ...
      await db.rpc(
        'room_set_visibility',
        { room_id: started.room.id, visibility: 'extended', join_policy: 'friends' },
        owner.as,
      )
      ...
      await db.expectError(
        db.rpc(
          'room_join',
          { room_id: started.room.id, media_state: 'camera', consent_level: 'extended' },
          friendOfFriend.as,
        ),
        'join_not_allowed',
      )
      ...
      await db.rpc(
        ...
        { room_id: started.room.id, join_policy: 'friends_of_friends' },
        owner.as,
      )
      ...
      await db.rpc(
        'room_set_join_policy',
        { room_id: started.room.id, join_policy: 'friends' },
        owner.as,
      )
      const joined = await joinRoom(db, started.room.id, friendOfOwner, 'camera', 'world')
      expect(joined.myParticipant?.status).toBe('active')
      expect(
        (await joinRoom(db, started.room.id, friendOfFriend, 'camera', 'world')).myParticipant
          ?.status,
      ).toBe('active')
      // A stranger cannot see an extended room at all.
      await db.expectError(
        db.rpc('room_get', { room_id: started.room.id }, stranger.as),
        'room_not_found',
      )
      ...
      const opened = RoomVisibilityChangeDtoSchema.parse(
        await db.rpc(
          'room_set_visibility',
          { room_id: started.room.id, visibility: 'world', join_policy: 'request' },
          owner.as,
        ),
      )
      ...
      const admitted = RoomDtoSchema.parse(
        await db.rpc(
          'room_admit',
          { room_id: started.room.id, participant_id: waiting.myParticipant?.id },
          owner.as,
        ),
      )
      expect(admitted.participants.find((p) => p.humanId === stranger.humanId)?.status).toBe(
        'active',
      )

      await db.rpc(
        'room_set_join_policy',
        { room_id: started.room.id, join_policy: 'anyone_with_link' },
        owner.as,
      )
      ...
      await db.expectError(
        db.rpc(
          'room_join',
          { room_id: started.room.id, media_state: 'camera', consent_level: 'world' },
          another.as,
        ),
        'join_not_allowed',
      )
      ...
      await db.expectError(
        db.rpc(
          'room_set_join_policy',
          { room_id: started.room.id, join_policy: 'group' },
          owner.as,
        ),
        'invalid_input',
      )
    ...
    it('group join policy admits members only; removed participants can never rejoin', async () => {
      ...
      await db.rpc(
        'room_set_visibility',
        { room_id: started.room.id, visibility: 'friends', join_policy: 'group' },
        owner.as,
      )
      await db.expectError(
        db.rpc(
      ...
      await db.expectError(
        db.rpc(
          'room_remove_participant',
          { room_id: started.room.id, participant_id: NIL_UUID },
          owner.as,
        ),
        'not_in_room',
      )
      await db.expectError(
        db.rpc(
          'room_remove_participant',
          { room_id: started.room.id, participant_id: participantId(removed, owner.humanId) },
          owner.as,
        ),
        'invalid_input',
      )
  ...
  describe('leaving, moderator transfer, ending', () => {
    it('sole moderator leaving transfers to the earliest active verified Human, never to a guest', async () => {
      ...
      expect(await participantStatus(db, roomId, early.humanId)).toMatchObject({
        role: 'moderator',
      })
      ...
      const second = RoomLeaveDtoSchema.parse(
        await db.rpc('room_leave', { room_id: roomId }, late.as),
      )
      expect(second.transferredTo).toBeNull()
      // Last Human leaves: only the guest remains; nobody becomes moderator.
      const third = RoomLeaveDtoSchema.parse(
        await db.rpc('room_leave', { room_id: roomId }, early.as),
      )
      ...
      expect(
        await count(
          ...
          "room_id = $1 and role in ('initiator', 'moderator') and status = 'active'",
          [roomId],
        ),
      ).toBe(0)
      ...
      const swept = await rpcAt<{ roomsEnded: number }>(
        db,
        'rooms_sweep',
        {},
        'service',
    ...
    it('room_end by the moderator ends immediately and clears pointers; non-moderators cannot end', async () => {
      ...
      await joinRoom(db, started.room.id, member, 'watching')
      await db.expectError(
        db.rpc('room_end', { room_id: started.room.id }, member.as),
        'not_a_moderator',
      )
      await db.expectError(
        db.rpc('room_end', { room_id: started.room.id }, outsider.as),
        'room_not_found',
      )
      const ended = RoomDtoSchema.parse(
        await db.rpc('room_end', { room_id: started.room.id, reason: 'done' }, owner.as),
      )
      ...
      expect(await roomRow(db, started.room.id)).toMatchObject({
        ended_reason: 'done',
  ...
  describe('room_media_grant', () => {
    it('returns MediaGrantDto for active participants and room_ended for ended rooms', async () => {
      ...
      const grant = MediaGrantDtoSchema.parse(
        await db.rpc('room_media_grant', { room_id: started.room.id }, owner.as),
      )
      ...
      await joinRoom(db, started.room.id, member, 'watching')
      const viewer = MediaGrantDtoSchema.parse(
        await db.rpc('room_media_grant', { room_id: started.room.id }, member.as),
      )
      expect(viewer).toMatchObject({ role: 'viewer', canPublish: false, canSubscribe: true })
      await db.expectError(
        db.rpc('room_media_grant', { room_id: started.room.id }, outsider.as),
        'not_in_room',
      )
      await db.expectError(
        db.rpc('room_media_grant', { room_id: started.room.id }, 'visitor'),
        'not_authenticated',
      )
      await db.rpc('room_end', { room_id: started.room.id }, owner.as)
      await db.expectError(
        db.rpc('room_media_grant', { room_id: started.room.id }, owner.as),
        'room_ended',
      )
    })
  })
  ...
  describe('room_participant_sync (service) and rooms_sweep', () => {
    it('reconciles LiveKit events, ignores out-of-order ones, transfers moderation', async () => {
      ...
      await expect(
        db.rpc(
          ...
          { room_id: roomId, livekit_identity: `h:${owner.humanId}`, event: 'participant_left' },
          owner.as,
        ),
      ).rejects.toMatchObject({ code: '42501' })
      const leftAt = secondsFromNow(0)
      const left = await db.rpc<{
        applied: boolean
        ignored: boolean
        ...
        {
          room_id: roomId,
          livekit_identity: `h:${owner.humanId}`,
      ...
      await db.expectError(
        db.rpc(
          'room_participant_sync',
          { room_id: roomId, livekit_identity: 'x', event: 'danced' },
          'service',
        ),
        'invalid_input',
      )
    ...
    it('rooms_sweep ends guest-only rooms after the grace period and empty rooms right away', async () => {
      ...
      const early = await rpcAt<{ roomsEnded: number }>(
        db,
        'rooms_sweep',
        {},
        'service',
      ...
      const late = await rpcAt<{ roomsEnded: number }>(
        db,
        'rooms_sweep',
        {},
        'service',
      ...
      expect(swept.roomsEnded).toBe(1)
      expect(await roomRow(db, empty.room.id)).toMatchObject({
        status: 'ended',
        ended_reason: 'empty',
      })
      ...
      const none = await rpcAt<{ roomsEnded: number }>(
        db,
        'rooms_sweep',
        {},
        'service',
    ...
    it('a block inside a live room clears the blocked seat; the blocker keeps their room (0360)', async () => {
      ...
      await block(db, a, b)
      expect(await participantStatus(db, started.room.id, b.humanId)).toMatchObject({
        status: 'removed',
      })
      ...
      await block(db, d, c)
      expect(await participantStatus(db, second.room.id, d.humanId)).toMatchObject({
        status: 'left',
      })
      expect(await participantStatus(db, second.room.id, c.humanId)).toMatchObject({
        status: 'active',
        role: 'initiator',
      })
