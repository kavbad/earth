import { ConversationDetailDtoSchema, ConversationSummaryDtoSchema, GroupDetailDtoSchema } from '@earth/domain'
...
describe('activeRoom pointers on chats and groups (0350; DB_API §2)', () => {
  ...
  it('group_get, conversation_get and conversations_list carry {roomId, participantCount} while the room is live', async () => {
    const before = GroupDetailDtoSchema.parse(await db.rpc('group_get', { group_id: group.groupId }, member.as))
    ...
    const detail = GroupDetailDtoSchema.parse(await db.rpc('group_get', { group_id: group.groupId }, member.as))
    ...
    expect(summaries.find((c) => c.id === group.conversationId)?.activeRoom).toEqual({ roomId, participantCount: 2 })

    // The pointer follows the trigger-maintained count.
    await db.rpc('room_leave', { room_id: roomId }, member.as)
    expect(GroupDetailDtoSchema.parse(await db.rpc('group_get', { group_id: group.groupId }, owner.as)).activeRoom).toEqual({
      roomId,
      participantCount: 1,
    })

    await db.rpc('room_end', { room_id: roomId }, owner.as)
    expect(GroupDetailDtoSchema.parse(await db.rpc('group_get', { group_id: group.groupId }, owner.as)).activeRoom).toBeNull()
    expect(
      ConversationDetailDtoSchema.parse(await db.rpc('conversation_get', { conversation_id: group.conversationId }, owner.as)).activeRoom,
    ).toBeNull()
  ...
  it('direct rooms show on the direct conversation for both members and only while live', async () => {
    ...
    expect(
      ConversationDetailDtoSchema.parse(await db.rpc('conversation_get', { conversation_id: conversationId }, member.as)).activeRoom,
    ...
    expect(await scalar(db, 'active_room_id from public.conversations where id = $1', [conversationId])).toBeNull()
  })

  it('a stale pointer (room ended out of band) is healed by the sweep and never surfaces as activeRoom', async () => {
    const started = await startGroupRoom(db, owner, group)
    await db.sql.query(`update public.rooms set status = 'ended', ended_at = now(), ended_reason = 'oob' where id = $1`, [started.room.id])
    expect(GroupDetailDtoSchema.parse(await db.rpc('group_get', { group_id: group.groupId }, owner.as)).activeRoom).toBeNull()
