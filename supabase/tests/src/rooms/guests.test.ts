...
describe('guests and room invites (spec §34–§35, §42–§43, SCREEN 17–19; DB_API §3)', () => {
  ...
  describe('room_invite_create / room_invite_preview', () => {
    it('active Human participants create links; the token is returned once and stored hashed', async () => {
      ...
      const invite = RoomInviteCreateDtoSchema.parse(
        await db.rpc(
          'room_invite_create',
          { room_id: started.room.id, expires_in_seconds: 3600 },
          host.as,
        ),
      )
      ...
      expect(
        await scalar(db, 'token_hash from public.room_invites where room_id = $1', [
          started.room.id,
        ]),
      ).toMatch(/^[0-9a-f]{64}$/)
      // Seeing the room is not enough: only active participants create links; strangers do not see it.
      await db.expectError(
        db.rpc('room_invite_create', { room_id: started.room.id }, cohost.as),
        'not_in_room',
      )
      const stranger = await human(db, 'Stranger')
      await db.expectError(
        db.rpc('room_invite_create', { room_id: started.room.id }, stranger.as),
        'room_not_found',
      )
      await db.expectError(
        db.rpc(
          'room_invite_create',
          { room_id: started.room.id, expires_in_seconds: 48 * 3600 },
          host.as,
        ),
        'invalid_input',
      )
      await db.expectError(
        db.rpc('room_invite_create', { room_id: started.room.id }, 'visitor'),
        'not_authenticated',
      )
      ...
      await db.expectError(
        db.rpc('room_invite_create', { room_id: started.room.id }, guest.as),
        'guest_not_allowed',
      )
      await db.rpc('room_end', { room_id: started.room.id }, host.as)
      await db.expectError(
        db.rpc('room_invite_create', { room_id: started.room.id }, host.as),
        'room_ended',
      )
    })
    ...
    it('preview shows publishers, context, join policy, guestsAllowed and ended (RoomInvitePreviewDto)', async () => {
      ...
      const preview = RoomInvitePreviewDtoSchema.parse(
        await db.rpc('room_invite_preview', { token }, 'visitor'),
      )
      ...
      expect(
        RoomInvitePreviewDtoSchema.parse(await db.rpc('room_invite_preview', { token }, 'visitor'))
          .guestsAllowed,
      ).toBe(false)
      ...
      await db.expectError(
        db.rpc('room_invite_preview', { token: 'nope' }, 'visitor'),
        'invite_invalid',
      )
      await db.rpc('room_end', { room_id: roomId }, host.as)
      const ended = RoomInvitePreviewDtoSchema.parse(
        await db.rpc('room_invite_preview', { token }, cohost.as),
      )
  ...
  describe('guest_session_create', () => {
    it('requires an anonymous auth user', async () => {
      const { roomId, token } = await liveRoom()
      await db.expectError(
        db.rpc('guest_session_create', { token, display_name: 'Sam' }, 'visitor'),
        'not_authenticated',
      )
      await db.expectError(
        db.rpc('guest_session_create', { token, display_name: 'Sam' }, cohost.as),
        'forbidden',
      )
      const unclaimed = await createUnclaimed(db)
      await db.expectError(
        db.rpc('guest_session_create', { token, display_name: 'Sam' }, unclaimed.as),
        'forbidden',
      )
      const guest = await createGuest(db)
      ...
      expect(
        await scalar(db, 'session_secret_hash from public.guest_sessions where id = $1', [
          session.guestSessionId,
        ]),
      ).toMatch(/^[0-9a-f]{64}$/)
      expect(
        await scalar(db, 'auth_user_id from public.guest_sessions where id = $1', [
          session.guestSessionId,
        ]),
      ).toBe(guest.userId)
      ...
      expect(
        await scalar(db, 'use_count from public.room_invites where room_id = $1', [roomId]),
      ).toBe(1)
      await db.rpc('room_end', { room_id: roomId }, host.as)
    })
    ...
    it('requires an active room, GUEST_ROOMS_ENABLED, guests not disabled and a usable invite', async () => {
      ...
      const guest = await createGuest(db)
      await db.expectError(
        db.rpc('guest_session_create', { token, display_name: '' }, guest.as),
        'invalid_input',
      )
      await db.expectError(
        db.rpc('guest_session_create', { token: 'nope', display_name: 'Sam' }, guest.as),
        'invite_invalid',
      )
      await setFlag(db, 'GUEST_ROOMS_ENABLED', false)
      await db.expectError(
      ...
      await expect(
        rpcAt(
          ...
          { token: expired.token, display_name: 'Sam' },
          guest.as,
          secondsFromNow(120),
        ),
    ...
    it('is rate limited to 5 per 10 minutes for a Guest credential (half of 10)', async () => {
      ...
      expect(
        await count(db, 'public.guest_sessions', 'room_id = $1 and auth_user_id = $2', [
          roomId,
          guest.userId,
        ]),
      ).toBe(1)
  ...
  describe('guest in room (SCREEN 18)', () => {
    ...
    beforeAll(async () => {
      ...
      sessionId = (
        await createGuestSession(db, guest, token, 'Sam', { fingerprint: 'fp-guest-1234' })
      ).guestSessionId
    })
    ...
    it('gets a media grant with identity g:<session id> and can change media state', async () => {
      ...
      expect(
        await scalar(
          db,
          'media_state::text from public.room_participants where guest_session_id = $1',
          [sessionId],
        ),
      ).toBe('camera')
    ...
    it('cannot expand visibility, create links, consent, or discover Lives (guest_not_allowed)', async () => {
      await db.expectError(
        db.rpc('room_set_visibility', { room_id: roomId, visibility: 'world' }, guest.as),
        'guest_not_allowed',
      )
      await db.expectError(
        db.rpc('room_invite_create', { room_id: roomId }, guest.as),
        'guest_not_allowed',
      )
      await db.expectError(
        db.rpc('room_consent', { room_id: roomId, level: 'world' }, guest.as),
        'guest_not_allowed',
      )
      await db.expectError(
        db.rpc('room_set_join_policy', { room_id: roomId, join_policy: 'anyone' }, guest.as),
        'guest_not_allowed',
      )
      await db.expectError(db.rpc('room_end', { room_id: roomId }, guest.as), 'guest_not_allowed')
      await db.expectError(
        db.rpc('live_candidates', { scope: 'world' }, guest.as),
        'guest_not_allowed',
      )
      await db.expectError(
        db
          .rpc('room_join', { room_id: roomId, media_state: 'audio' }, guest.as)
          .then(() => db.rpc('room_start', { context_type: 'standalone' }, guest.as)),
        'not_a_human',
      )
    })
    ...
    it('can leave and come back through room_join while the session is usable', async () => {
      ...
      expect(
        await scalar(db, 'status::text from public.room_participants where guest_session_id = $1', [
          sessionId,
        ]),
      ).toBe('left')
      expect(await roomRow(db, roomId)).toMatchObject({ active_participant_count: 1 })
      const back = RoomDtoSchema.parse(
        await db.rpc('room_join', { room_id: roomId, media_state: 'audio' }, guest.as),
      )
      expect(back.myParticipant).toMatchObject({
      ...
      expect(
        await count(db, 'public.room_participants', "guest_session_id = $1 and status = 'active'", [
          sessionId,
        ]),
      ).toBe(1)
      expect(
        await count(db, 'public.room_participants', 'guest_session_id = $1', [sessionId]),
      ).toBe(2)
      await db.expectError(
        db.rpc('room_leave', { room_id: roomId }, await createGuest(db).then((g) => g.as)),
        'not_in_room',
      )
    })
    ...
    it('guest_session_get counts distinct rooms joined and Humans met', async () => {
      ...
      const before = await db.rpc<{
        roomsJoined: number
        humansMet: number
        current: unknown
        sessions: unknown[]
      }>('guest_session_get', {}, guest.as)
      expect(before).toMatchObject({ roomsJoined: 1, humansMet: 2 })
      expect(GuestSessionDtoSchema.parse(before.current)).toMatchObject({
        guestSessionId: sessionId,
        roomId,
      })
      ...
      const after = await db.rpc<{ roomsJoined: number; humansMet: number }>(
        'guest_session_get',
        {},
        guest.as,
      )
    ...
    it('moderator removal with block_from_room blocks the fingerprint and the credential for new sessions', async () => {
      ...
      await db.expectError(
        db.rpc(
          ...
          { room_id: roomId, participant_id: participant?.id, block_from_room: true },
          cohost.as,
        ),
        'not_a_moderator',
      )
      ...
      expect(
        await scalar(
          db,
          'status::text from public.room_participants where guest_session_id = $1 order by joined_at desc limit 1',
          [sessionId],
        ),
      ).toBe('removed')
      expect(
        await scalar(db, 'removed_at is not null from public.guest_sessions where id = $1', [
          sessionId,
        ]),
      ).toBe(true)
      expect(
        await count(
          db,
          'public.room_blocked_fingerprints',
          'room_id = $1 and fingerprint_hash = $2',
          [roomId, 'fp-guest-1234'],
        ),
      ).toBe(1)
      ...
      await db.expectError(
        db.rpc(
          'guest_session_create',
          { token, display_name: 'Sam', device_fingerprint_hash: 'fp-guest-1234' },
          newDevice.as,
        ),
        'blocked',
      )
      ...
      expect(
        await count(
          db,
          'private.audit_log',
          "action = 'room_remove_participant' and target_id = $1",
          [roomId],
        ),
      ).toBe(1)
    })
    ...
    it('disabling guests removes every active guest and stops new sessions', async () => {
      const disabled = RoomDtoSchema.parse(
        await db.rpc('room_set_guests_disabled', { room_id: roomId, disabled: true }, host.as),
      )
      ...
      expect(
        await count(
          ...
          "room_id = $1 and guest_session_id is not null and status = 'active'",
          [roomId],
        ),
      ).toBe(0)
      await db.expectError(
        db.rpc(
          ...
          await createGuest(db).then((g) => g.as),
        ),
        'guests_disabled',
      )
      await db.rpc('room_set_guests_disabled', { room_id: roomId, disabled: false }, host.as)
  ...
  describe('guest session expiry (spec §34 "expires with room plus a short grace period")', () => {
    it('ending the room expires guest sessions after the grace; the guest loses the room afterwards', async () => {
      ...
      const expiresAt = await scalar<Date>(
        db,
        'expires_at from public.guest_sessions where id = $1',
        [session.guestSessionId],
      )
      ...
      const current = await rpcAt<{ current: unknown }>(
        db,
        'guest_session_get',
        {},
        guest.as,
    ...
    it('rooms_sweep expires sessions past their expiry while the room is still open', async () => {
      ...
      const guest = await createGuest(db)
      const session = await createGuestSession(db, guest, token, 'Sam')
      const swept = await rpcAt<{ guestSessionsExpired: number }>(
        db,
        'rooms_sweep',
        {},
        'service',
        secondsFromNow(25 * 3600),
      )
      ...
      await expect(
        rpcAt(db, 'room_media_grant', { room_id: roomId }, guest.as, secondsFromNow(25 * 3600)),
      ).rejects.toMatchObject({ message: 'not_in_room' })
      await db.rpc('room_end', { room_id: roomId }, host.as)
    })
  })
})
