...
describe('analytics_track (spec §96–§97; DB_API §8)', () => {
  ...
  describe('identity attribution', () => {
    it('visitor: anonymousVisitorId (top-level or merged into properties) is stored; nothing else', async () => {
      ...
      const topLevel = await trackOne(db, 'visitor', event('public_world_viewed', { surface: 'home', scope: 'world' }, { anonymousVisitorId: visitor }))
      ...
      const merged = await trackOne(db, 'visitor', event('claim_started', { entry: 'public_world', hasGroupInvite: false, anonymousVisitorId: visitor }))
      expect(merged.anonymous_visitor_id).toBe(visitor)
      expect(merged.properties).toEqual(expect.not.objectContaining({ anonymousVisitorId: expect.anything() }))

      const bare = await trackOne(db, 'visitor', event('feed_opened', { scope: 'world', surface: 'home', source: 'launch' }))
      expect(bare).toMatchObject({ human_id: null, anonymous_visitor_id: null, guest_session_id: null })
    })

    it('a client-supplied humanId is never trusted: visitors stay anonymous, Humans are themselves', async () => {
      const spoofed = await trackOne(db, 'visitor', event('feed_opened', { scope: 'world', surface: 'home', source: 'launch', humanId: alice.humanId }, { humanId: alice.humanId }))
      expect(spoofed.human_id).toBeNull()
      expect(spoofed.properties).not.toHaveProperty('humanId')

      const own = await trackOne(db, alice.as, event('post_opened', { postId: NIL_UUID, source: 'home', humanId: bob.humanId }, { humanId: bob.humanId, anonymousVisitorId: visitorId() }))
      expect(own.human_id).toBe(alice.humanId)
      expect(own.anonymous_visitor_id).not.toBeNull()
      expect(own.properties).toEqual({ postId: NIL_UUID, source: 'home', marker: expect.any(String) })
    })
    ...
    it('claiming Humans are attributed to their pending Human; an unclaimed credential to nobody', async () => {
      ...
      const claimEvent = await trackOne(db, pending.as, event('claim_started', { entry: 'launch', hasGroupInvite: false }))
      expect(claimEvent.human_id).toBe(pending.humanId)

      const unclaimed = await createUnclaimed(db)
      const unclaimedEvent = await trackOne(db, unclaimed.as, event('claim_started', { entry: 'launch', hasGroupInvite: false }))
      expect(unclaimedEvent.human_id).toBeNull()

      const restricted = await createHuman(db, { handle: 'restrictedone', status: 'restricted' })
      const restrictedEvent = await trackOne(db, restricted.as, event('feed_opened', { scope: 'friends', surface: 'home', source: 'tab' }))
    ...
    it('the service may call the RPC (no Human) and insert directly', async () => {
      const viaRpc = await trackOne(db, 'service', event('room_created', { roomId: NIL_UUID, contextType: 'group', visibility: 'group', joinPolicy: 'group' }, { platform: 'server' }))
      ...
      expect(await count(db, 'public.analytics_events', "name = 'group_created' and human_id = $1", [alice.humanId])).toBe(1)
    })
    ...
    describe('guests', () => {
      ...
      it('attaches the supplied guestSessionId only when it belongs to the caller', async () => {
        const own = await trackOne(db, guestA.as, event('guest_joined', { roomId: roomA, guestSessionId: sessionA, mediaState: 'audio' }))
        ...
        const topLevel = await trackOne(db, guestA.as, event('guest_room_completed', { durationMs: 10, outcome: 'left' }, { guestSessionId: sessionA }))
        expect(topLevel.guest_session_id).toBe(sessionA)

        const foreign = await trackOne(db, guestB.as, event('guest_room_completed', { guestSessionId: sessionA, durationMs: 10, outcome: 'left' }))
        expect(foreign.guest_session_id).toBeNull()
      })

      it('falls back to the caller session of properties.roomId; otherwise null', async () => {
        const byRoom = await trackOne(db, guestB.as, event('room_left', { roomId: roomB, durationMs: 5, reason: 'left' }))
        expect(byRoom.guest_session_id).toBe(sessionB)
        const otherRoom = await trackOne(db, guestB.as, event('room_left', { roomId: roomA, durationMs: 5, reason: 'left' }))
        expect(otherRoom.guest_session_id).toBeNull()
        const noRoom = await trackOne(db, guestB.as, event('guest_room_opened', { viewerState: 'guest' }))
        ...
        const mixed = await trackOne(db, guestB.as, event('guest_room_completed', { roomId: roomB, guestSessionId: sessionA, durationMs: 1, outcome: 'left' }))
      ...
      it('a Guest supplying a malformed guestSessionId gets invalid_input; a Human never attaches one', async () => {
        await db.expectError(track(db, [event('guest_joined', { guestSessionId: 'garbage' })], guestA.as), 'invalid_input')
        const asHuman = await trackOne(db, alice.as, event('human_claimed', { intent: 'join_group', guestSessionId: sessionA, durationMs: 100 }, { guestSessionId: sessionA }))
  ...
  describe('properties', () => {
    it('drops the reserved base and identity keys (guestSessionId stays) and reads platform/appVersion from them', async () => {
      ...
      expect(row).toMatchObject({ platform: 'ios', app_version: '2.3.4', client_timestamp: '2026-09-03T10:00:00+00:00' })
      expect(Object.keys(row.properties).sort()).toEqual(['guestSessionId', 'marker', 'scope', 'source', 'surface'])
    })

    it('strips keys that name a coordinate and values that read like one, recursively', async () => {
      const row = await trackOne(db, alice.as, event('post_created', {
        postId: NIL_UUID,
        type: 'text',
        audience: 'world',
        hasMedia: false,
    ...
    it('clientTimestamp: top-level wins over properties.timestamp; malformed values are refused', async () => {
      const both = await trackOne(db, alice.as, event('feed_opened', { scope: 'city', surface: 'home', source: 'tab', timestamp: '2026-09-03T09:00:00Z' }, { clientTimestamp: '2026-09-03T10:30:00.250+02:00' }))
      expect(both.client_timestamp).toBe('2026-09-03T08:30:00.25+00:00')
      const merged = await trackOne(db, alice.as, event('feed_opened', { scope: 'city', surface: 'home', source: 'tab', timestamp: '2026-09-03T09:00:00Z' }))
      expect(merged.client_timestamp).toBe('2026-09-03T09:00:00+00:00')
      await db.expectError(track(db, [event('feed_opened', {}, { clientTimestamp: 'not-a-date' })], alice.as), 'invalid_input')
      await db.expectError(track(db, [event('feed_opened', {}, { clientTimestamp: 1725000000 })], alice.as), 'invalid_input')
    })
  })
  ...
  describe('validation (invalid_input)', () => {
    const cases: Array<[string, unknown]> = [
      ...
      ['a non-uuid anonymousVisitorId', [event('feed_opened', {}, { anonymousVisitorId: 'device-1' })]],
      ['a non-uuid anonymousVisitorId in properties', [event('feed_opened', { anonymousVisitorId: 12 })]],
      ['more than 64 properties', [event('feed_opened', Object.fromEntries(Array.from({ length: 65 }, (_, i) => [`k${i}`, i])))]],
    ]
    ...
    it('refuses a null argument and stores nothing from a partially valid batch', async () => {
      ...
      await db.expectError(
        track(db, [event('feed_opened', { marker: 'partial' }), event('feed_opened', {}, { platform: 'nope' })], alice.as),
        'invalid_input',
      )
    ...
    it('accepts exactly 50 events and a 64-character appVersion', async () => {
      ...
      expect(await track(db, [event('feed_opened', {}, { appVersion: 'v'.repeat(64) })], bob.as)).toEqual({ accepted: 1 })
    })
  })

  describe('rate limit (600 events / 10 min per Human, half for Guests and Visitors, never the service)', () => {
    const fill = async (as: RoleSpec, budget: number, headers?: Record<string, string>): Promise<void> => {
      for (let sent = 0; sent < budget; sent += TRACK_BATCH_MAX) {
        const size = Math.min(TRACK_BATCH_MAX, budget - sent)
        expect(await track(db, batch(size), as, headers === undefined ? {} : { headers })).toEqual({ accepted: size })
      }
    }
    ...
    it('a Human gets 600 events per window; the window rolls over', async () => {
      ...
      expect(await count(db, 'public.analytics_events', 'human_id = $1', [limited.humanId])).toBe(TRACK_BUDGET)
      ...
      expect(await track(db, batch(TRACK_BATCH_MAX), limited.as, { at: later })).toEqual({ accepted: TRACK_BATCH_MAX })
    })
    ...
    it('a Visitor is keyed by client address with half the budget; another address is unaffected', async () => {
      ...
      await db.expectError(track(db, [event('feed_opened')], 'visitor', { headers: ip }), 'rate_limited')
      expect(await track(db, [event('feed_opened')], 'visitor', { headers: { 'cf-connecting-ip': '203.0.113.11' } })).toEqual({ accepted: 1 })
    })
