...
describe('metrics_compute_daily (spec §98–§101; DB_API §8)', () => {
  ...
  const compute = (day: string | null) => db.rpc<{ day: string; computedAt: string; metrics: unknown[] }>('metrics_compute_daily', { day }, 'service')

  async function setClaim(h: Human, intent: 'start_group' | 'join_group' | null, claimedAt: string): Promise<void> {
    await db.sql.query('update public.humans set claim_intent = $2, claimed_at = $3 where id = $1', [h.humanId, intent, claimedAt])
  }
  ...
  async function groupAt(owner: Human, name: string, createdAt: string): Promise<GroupFixture> {
    ...
    await db.sql.query('update public.group_members set joined_at = $3 where group_id = $1 and human_id = $2', [group.groupId, owner.humanId, createdAt])
    return group
  }

  async function memberAt(group: GroupFixture, member: Human, joinedAt: string, leftAt: string | null = null): Promise<void> {
    await addMember(db, group, member)
    await db.sql.query('update public.group_members set joined_at = $3 where group_id = $1 and human_id = $2', [group.groupId, member.humanId, joinedAt])
    if (leftAt !== null) {
      await db.sql.query(`update public.group_members set status = 'left', left_at = $3 where group_id = $1 and human_id = $2`, [group.groupId, member.humanId, leftAt])
    }
  }

  async function messageAt(conversationId: string, sender: Human, createdAt: string, type: 'text' | 'system' = 'text'): Promise<void> {
    await db.sql.query(
      `insert into public.messages (conversation_id, sender_human_id, type, text, created_at) values ($1, $2, $3::public.message_type, $4, $5)`,
      [conversationId, sender.humanId, type, type === 'system' ? 'Someone joined' : 'hello', createdAt],
    )
  }

  async function eventAt(as: Human | 'visitor', name: string, properties: Record<string, unknown>, createdAt: string, extra: Record<string, unknown> = {}): Promise<void> {
    const marker = randomUUID()
    await track(db, [event(name, { ...properties, marker }, extra)], as === 'visitor' ? 'visitor' : as.as)
    await db.sql.query(`update public.analytics_events set created_at = $2 where properties ->> 'marker' = $1`, [marker, createdAt])
  }

  /** Full claim flow (DB_API §1) for a fresh credential; returns the active Human. */
  async function claim(handle: string, intent: 'start_group' | 'join_group', option: { groupLabel?: string; inviteToken?: string }): Promise<Human> {
    const user = await createUnclaimed(db)
    const started = await db.rpc<{ humanId: string }>('claim_start', { intent, group_label: option.groupLabel ?? null, invite_token: option.inviteToken ?? null }, user.as)
    ...
    await db.rpc(
      ...
      { human_id: started.humanId, status: 'verified', risk_level: 'low', provider: 'mock', provider_reference: `sess-${handle}`, metadata: { provider: 'mock' }, duplicate_of_human_id: null },
      'service',
    )
    await db.rpc('claim_complete', {}, user.as)
    return { userId: user.userId, humanId: started.humanId, handle, displayName: handle, as: user.as }
  }
  ...
  beforeAll(async () => {
    ...
    const ge = await db.rpc<{ id: string; groupId: string }>('conversation_group_create', { human_ids: [m1.humanId, m2.humanId] }, hostA.as)
    await setCreatedAt(db, 'public.groups', ge.groupId, at(0, '11:00'))
    expect(await scalar(db, 'kind::text from public.groups where id = $1', [ge.groupId])).toBe('temporary')
    ...
    await db.sql.query('update public.group_members set joined_at = $3 where group_id = $1 and human_id = $2', [gg.groupId, h2.humanId, at(0, '11:00')]) // GG: 2 at the cutoff → not
    ...
    expect(await scalar(db, 'claim_intent from public.humans where id = $1', [h1.humanId])).toBe('start_group')

    // --- claim_started: 4 visitors (one twice) + one Claiming Human without a visitor id = 5 ---
    const v1 = visitorId()
    await eventAt('visitor', 'claim_started', { entry: 'public_world', hasGroupInvite: false }, at(0, '08:00'), { anonymousVisitorId: v1 })
    await eventAt('visitor', 'claim_started', { entry: 'group_invite', hasGroupInvite: true }, at(0, '08:05'), { anonymousVisitorId: v1 })
    for (const time of ['08:10', '08:20', '08:30']) {
      await eventAt('visitor', 'claim_started', { entry: 'public_world', hasGroupInvite: false }, at(0, time), { anonymousVisitorId: visitorId() })
    }
    await eventAt(h4, 'claim_started', { entry: 'launch', hasGroupInvite: false }, at(0, '08:40'))
    await eventAt('visitor', 'claim_started', { entry: 'launch', hasGroupInvite: false }, at(-1, '08:00'), { anonymousVisitorId: visitorId() })
    await eventAt('visitor', 'claim_started', { entry: 'launch', hasGroupInvite: false }, at(1, '08:00'), { anonymousVisitorId: visitorId() })
    ...
    const temporary = await db.rpc<{ groupId: string }>('conversation_group_create', { human_ids: [s3.humanId, s2.humanId] }, hostB.as)
    ...
    for (const time of ['09:00', '10:00', '11:00', '12:00']) await messageAt(ga.conversationId, hostA, at(0, time))
    ...
    const dm = await db.rpc<{ id: string }>('conversation_direct_get_or_create', { other_human_id: hostB.humanId }, hostA.as)
    ...
    await eventAt(h2, 'human_claimed', { intent: 'join_group', guestSessionId: gB1, durationMs: 1200 }, at(0, '11:01'))
    await eventAt(h3, 'human_claimed', { intent: 'join_group', guestSessionId: randomUUID(), durationMs: 900 }, at(0, '12:01'))
    await eventAt(h5, 'human_claimed', { intent: 'start_group', guestSessionId: 'not-a-uuid', durationMs: 800 }, at(0, '13:01'))
    await eventAt(h4, 'human_claimed', { intent: 'join_group', guestSessionId: gB1, durationMs: 700 }, at(-1, '12:01'))

    // --- Scope switches: 3 by h1 + 1 by h3 on D; one on D-1 ---
    for (const time of ['15:00', '15:01', '15:02']) await eventAt(h1, 'scope_changed', { from: 'friends', to: 'city', surface: 'home' }, at(0, time))
    await eventAt(h3, 'scope_changed', { from: 'city', to: 'world', surface: 'live' }, at(0, '16:00'))
    await eventAt(h1, 'scope_changed', { from: 'world', to: 'friends', surface: 'earth' }, at(-1, '16:00'))
  ...
  it('group_seed_rate = Humans claimed ÷ distinct claim-intent Visitors (4 ÷ 5)', () => {
    expect(row('group_seed_rate')).toMatchObject({ value: 0.8, details: { humansClaimed: 4, claimIntentVisitors: 5 } })
  })
  ...
  it('group_activation_rate for cohorts D (1 of 4) and D-7 (1 of 2), persistent groups only', () => {
    expect(row('group_activation_rate', { cohort: D })).toMatchObject({ value: 0.25, details: { groupsCreated: 4, groupsActivated: 1, final: true } })
    expect(row('group_activation_rate', { cohort: dayOffset(-7) })).toMatchObject({ value: 0.5, details: { groupsCreated: 2, groupsActivated: 1, final: true } })
  })

  it('second_group_rate for the cohort claimed 30 days earlier (1 of 4)', () => {
    expect(row('second_group_rate', { cohort: dayOffset(-30) })).toMatchObject({ value: 0.25, details: { humansClaimed: 4, withSecondGroup: 1 } })
  })

  it('messages_per_active_group counts non-system group messages (6 ÷ 2)', () => {
    expect(row('messages_per_active_group')).toMatchObject({ value: 3, details: { messages: 6, activeGroups: 2 } })
  })

  it('groups_active_3_days_week counts groups with messages on ≥ 3 days of the trailing week (GA, GF)', () => {
    expect(row('groups_active_3_days_week')).toMatchObject({ value: 2, details: { windowStart: dayOffset(-6), windowEnd: D, groups: 2 } })
  })

  it('rooms_started and rooms_opened_beyond_group (3 rooms; R1 widened then narrowed, R2 friends)', () => {
    expect(row('rooms_started')).toMatchObject({ value: 3, details: { rooms: 3, byContextType: { group: 2, standalone: 1 } } })
    expect(row('rooms_opened_beyond_group')).toMatchObject({ value: 2, details: { rooms: 2, groupRooms: 1, roomsStarted: 3 } })
  })
  ...
  it('guest_joins, repeat_guests and guest_to_human_conversions', () => {
    ...
    expect(row('repeat_guests')).toMatchObject({ value: 1, details: { repeatGuests: 1, guestsInWindow: 3, windowStart: dayOffset(-6), windowEnd: D } })
    expect(row('guest_to_human_conversions')).toMatchObject({ value: 2, details: { conversions: 2, humansClaimed: 4 } })
  })

  it('scope_switches counts scope_changed events (4 by 2 Humans)', () => {
    expect(row('scope_switches')).toMatchObject({ value: 4, details: { switches: 4, distinctHumans: 2 } })
  })
  ...
  it('is idempotent: recomputing rewrites the same rows in place and picks up new data', async () => {
    ...
    for (const [key, previous] of before) {
      ...
      expect(Date.parse(current?.computed_at ?? '')).toBeGreaterThanOrEqual(Date.parse(previous.computed_at))
    }
    expect(await count(db, 'public.metrics_daily', 'day = $1', [D])).toBe(EXPECTED_ROWS)
    // New facts change the value under the same key.
    await eventAt(hostA, 'scope_changed', { from: 'friends', to: 'world', surface: 'home' }, at(0, '17:00'))
    await compute(D)
    expect((await metricMap(db, D)).get(metricKey('scope_switches'))).toMatchObject({ value: 5, details: { switches: 5, distinctHumans: 3 } })
  ...
  it('is service only and validates the day', async () => {
    ...
    await db.sql.query('grant execute on function public.metrics_compute_daily(date) to authenticated')
    await db.expectError(db.rpc('metrics_compute_daily', { day: D }, hostA.as), 'forbidden')
    await db.sql.query('revoke execute on function public.metrics_compute_daily(date) from authenticated')
  ...
  it('rooms.max_visibility is the widest visibility ever applied and never lowered', async () => {
    ...
    expect(await scalar(db, 'max_visibility::text from public.rooms where id = $1', [r1])).toBe('friends')
    ...
    expect(await scalar(db, 'max_visibility::text from public.rooms where id = $1', [r1])).toBe('city')
