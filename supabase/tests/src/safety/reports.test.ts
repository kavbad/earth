...
describe('reports (DB_API §7)', () => {
  ...
  describe('domain parity (packages/domain/src/enums.ts)', () => {
    it('earth.report_target_types() is exactly REPORT_TARGET_TYPES', async () => {
      const { rows } = await db.sql.query<{ types: string[] }>(
        'select earth.report_target_types() as types',
      )
    ...
    it('earth.report_high_severity_reasons() is exactly REPORT_REASON_HIGH_SEVERITY', async () => {
      const { rows } = await db.sql.query<{ reasons: string[] }>(
        'select earth.report_high_severity_reasons()::text[] as reasons',
      )
    ...
    it.each([...REPORT_REASON])(
      'severity of %s is derived from REPORT_REASON_HIGH_SEVERITY',
      async (reason) => {
        const expected = REPORT_REASON_HIGH_SEVERITY.has(reason) ? 'high' : 'normal'
        const { rows } = await db.sql.query<{ severity: string }>(
          'select earth.report_severity($1::public.report_reason) as severity',
          [reason],
        )
    ...
    it.each([...REPORT_TARGET_TYPES])(
      'target_type %s is accepted by the check constraint',
      async (targetType) => {
        await db.sql.query('begin')
        try {
  ...
  describe('report_create as a Human', () => {
    it('reports a visible Human: ReportDto, row, audit entry', async () => {
      const report = await createReport(db, alice.as, {
        targetType: 'human',
        targetId: bob.humanId,
        reason: 'harassment',
        details: '  keeps messaging me  ',
      })
      expect(ReportDtoSchema.parse(report)).toEqual({
        id: report.id,
        status: 'open',
    ...
    it('high-severity reasons are stored as severity high', async () => {
      ...
      expect((await auditRows(db, 'report_create', bob.humanId)).at(-1)?.details).toMatchObject({
        severity: 'high',
      })
    })
    ...
    it('validates the input', async () => {
      expect(
        await reportErrorCode(db, alice.as, { targetType: 'planet', targetId: bob.humanId }),
      ).toBe('invalid_input')
      expect(await reportErrorCode(db, alice.as, { targetType: 'human', targetId: null })).toBe(
        'invalid_input',
      )
      expect(
        await reportErrorCode(db, alice.as, {
          targetType: 'human',
          targetId: bob.humanId,
          details: 'x'.repeat(2001),
        }),
      ).toBe('invalid_input')
      const long = await createReport(db, alice.as, {
        targetType: 'human',
        targetId: bob.humanId,
        details: 'y'.repeat(2000),
      })
      expect(long.details).toHaveLength(2000)
      const blank = await createReport(db, alice.as, {
        targetType: 'human',
        targetId: bob.humanId,
        details: '   ',
      })
      ...
      try {
        await db.rpc(
          'report_create',
          { target_type: 'human', target_id: bob.humanId, reason: 'meh', details: null },
          alice.as,
        )
      } catch (error) {
        failure = error
    ...
    it('never reports yourself or your own content', async () => {
      expect(
        await reportErrorCode(db, alice.as, { targetType: 'human', targetId: alice.humanId }),
      ).toBe('invalid_input')
      const own = await createPost(db, alice, { audience: 'world', text: 'mine' })
      expect(
        await reportErrorCode(db, alice.as, { targetType: 'post', targetId: own.post.id }),
      ).toBe('invalid_input')
      const dm = await directConversation(db, alice, carol)
      const mine = await sendMessage(db, alice, dm, 'hello carol')
      expect(await reportErrorCode(db, alice.as, { targetType: 'message', targetId: mine })).toBe(
        'invalid_input',
      )
    })
    ...
    describe('Human targets', () => {
      it('unknown, pending or invisible Humans are not_visible', async () => {
        expect(
          await reportErrorCode(db, alice.as, { targetType: 'human', targetId: randomUUID() }),
        ).toBe('not_visible')
        const pending = await createHuman(db, { handle: 'pendingtarget', status: 'pending' })
        expect(
          await reportErrorCode(db, alice.as, { targetType: 'human', targetId: pending.humanId }),
        ).toBe('not_visible')
        const hidden = await createHuman(db, { handle: 'hiddenstranger', visibility: 'hidden' })
        expect(
          await reportErrorCode(db, alice.as, { targetType: 'human', targetId: hidden.humanId }),
        ).toBe('not_visible')
      })
      ...
      it('a hidden Human becomes reportable through a shared group, a conversation or a room', async () => {
        ...
        expect(
          await reportErrorCode(db, alice.as, {
            targetType: 'human',
            targetId: hiddenMate.humanId,
          }),
        ).toBe('not_visible')
        await addMember(db, crew, hiddenMate)
        expect(
          (await createReport(db, alice.as, { targetType: 'human', targetId: hiddenMate.humanId }))
            .targetId,
        ).toBe(hiddenMate.humanId)
        ...
        expect(
          (await createReport(db, alice.as, { targetType: 'human', targetId: hiddenChat.humanId }))
            .targetId,
        ).toBe(hiddenChat.humanId)
        ...
        expect(
          (
            await createReport(db, alice.as, {
              targetType: 'human',
              targetId: hiddenRoomie.humanId,
            })
          ).targetId,
        ).toBe(hiddenRoomie.humanId)
      })
      ...
      it('a Human you blocked stays reportable; one who blocked you without any shared context does not', async () => {
        ...
        expect(
          (
            await createReport(db, alice.as, {
              targetType: 'human',
              targetId: blockedByAlice.humanId,
            })
          ).targetId,
        ).toBe(blockedByAlice.humanId)

        const blocker = await human(db, 'Blocker')
        await block(db, blocker, alice)
        expect(
          await reportErrorCode(db, alice.as, { targetType: 'human', targetId: blocker.humanId }),
        ).toBe('not_visible')
        // ...but a blocker who still shares a group with the reporter is visible there.
        await addMember(db, crew, blocker)
        expect(
          (await createReport(db, alice.as, { targetType: 'human', targetId: blocker.humanId }))
            .targetId,
        ).toBe(blocker.humanId)
      })
    })

    describe('post targets', () => {
      it('follows earth.can_view_post', async () => {
        const friendsOnly = await createPost(db, bob, {
          audience: 'friends',
          text: 'friends of bob',
        })
        expect(
          await reportErrorCode(db, alice.as, {
            targetType: 'post',
            targetId: friendsOnly.post.id,
          }),
        ).toBe('not_visible')
        expect(
          (
            await createReport(db, carol.as, {
              targetType: 'post',
              targetId: (
                await createPost(db, alice, { audience: 'friends', text: 'friends of alice' })
              ).post.id,
            })
          ).targetType,
        ).toBe('post')
        ...
        expect((await auditRows(db, 'report_create', world.post.id))[0]).toMatchObject({
          target_type: 'post',
          actor_human_id: alice.humanId,
        })
        ...
        expect(
          await reportErrorCode(db, alice.as, { targetType: 'post', targetId: world.post.id }),
        ).toBe('not_visible')
        expect(
          await reportErrorCode(db, alice.as, { targetType: 'post', targetId: randomUUID() }),
        ).toBe('not_visible')
      })
    })
    ...
    describe('room targets', () => {
      it('follows earth.room_visible_to', async () => {
        ...
        expect(
          await reportErrorCode(db, alice.as, { targetType: 'room', targetId: theirRoom.room.id }),
        ).toBe('not_visible')

        const ours = await startGroupRoom(db, carol, crew)
        const report = await createReport(db, alice.as, {
          targetType: 'room',
          targetId: ours.room.id,
          reason: 'violence',
        })
        expect(report).toMatchObject({
          targetType: 'room',
        ...
        expect(
          await reportErrorCode(db, alice.as, { targetType: 'room', targetId: randomUUID() }),
        ).toBe('not_visible')
    ...
    describe('message targets', () => {
      it('follows earth.can_view_conversation', async () => {
        ...
        expect(
          await reportErrorCode(db, alice.as, { targetType: 'message', targetId: private_ }),
        ).toBe('not_visible')
        expect(
          await reportErrorCode(db, alice.as, { targetType: 'message', targetId: randomUUID() }),
        ).toBe('not_visible')
        ...
        expect(
          await reportErrorCode(db, alice.as, { targetType: 'message', targetId: fromEve }),
        ).toBe('not_visible')
      })
    })
    ...
    describe('group targets', () => {
      it('members only', async () => {
        const report = await createReport(db, carol.as, {
          targetType: 'group',
          targetId: crew.groupId,
          reason: 'impersonation',
        })
        expect(report).toMatchObject({ targetType: 'group', targetId: crew.groupId })
        expect(
          await reportErrorCode(db, bob.as, { targetType: 'group', targetId: crew.groupId }),
        ).toBe('not_visible')
        expect(
          await reportErrorCode(db, bob.as, { targetType: 'group', targetId: randomUUID() }),
        ).toBe('not_visible')
      })
    })
    ...
    describe('guest targets', () => {
      it('guests of rooms the reporter can see', async () => {
        ...
        const report = await createReport(db, alice.as, {
          targetType: 'guest',
          targetId: session.guestSessionId,
          reason: 'sexual_content',
        })
        expect(report).toMatchObject({ targetType: 'guest', targetId: session.guestSessionId })
        expect(
          await reportErrorCode(db, bob.as, {
            targetType: 'guest',
            targetId: session.guestSessionId,
          }),
        ).toBe('not_visible')
        expect(
          await reportErrorCode(db, alice.as, { targetType: 'guest', targetId: randomUUID() }),
        ).toBe('not_visible')
  ...
  describe('reports_mine', () => {
    it("lists the caller's own reports newest first and nothing else", async () => {
      ...
      const first = await rpcAt(
        db,
        'report_create',
        { target_type: 'human', target_id: bob.humanId, reason: 'spam_scam', details: null },
        reporter.as,
        secondsFromNow(-120),
      )
      const second = await createReport(db, reporter.as, {
        targetType: 'human',
        targetId: carol.humanId,
        reason: 'threats',
      })
    ...
    it('is for Humans only', async () => {
      ...
      expect(await errorCode(db.rpc('reports_mine', {}, (await createGuest(db)).as))).toBe(
        'not_a_human',
      )
      expect(await errorCode(db.rpc('reports_mine', {}, (await createUnclaimed(db)).as))).toBe(
        'not_a_human',
      )
    })
  })
  ...
  describe('blocks_list', () => {
    it('returns BlocksListDto with the blocked identities, newest first', async () => {
      ...
      const first = await createHuman(db, {
        handle: 'firstblocked',
        displayName: 'First',
        visibility: 'hidden',
      })
      ...
      await rpcAt(
        db,
        'block_set',
        { target_human_id: first.humanId, blocked: true },
        blocker.as,
        secondsFromNow(-60),
      )
      await db.sql.query(
        "update public.blocks set created_at = now() - interval '60 seconds' where blocker_human_id = $1 and blocked_human_id = $2",
        [blocker.humanId, first.humanId],
      )
      ...
      expect(list.blocks[0]).toMatchObject({
        blockerHumanId: blocker.humanId,
        identity: { humanId: second.humanId, displayName: 'SecondBlocked', handle: second.handle },
      })
      // A hidden profile is still listed to the Human who blocked it (they need to undo it).
      expect(list.blocks[1]?.identity).toMatchObject({
      ...
      expect((await blocksList(db, blocker.as)).blocks.map((b) => b.blockedHumanId)).toEqual([
        first.humanId,
      ])
    })
    ...
    it('is for Humans only', async () => {
      ...
      expect(await errorCode(db.rpc('blocks_list', {}, (await createGuest(db)).as))).toBe(
        'not_a_human',
      )
  ...
  describe('report_resolve (service only)', () => {
    it('moves a report through the queue and audits every step', async () => {
      ...
      expect(await resolveReport(db, report.id, 'open')).toMatchObject({
        status: 'open',
        resolvedAt: null,
      })
      ...
      expect(audit[0]).toMatchObject({
        ...
        actor_human_id: null,
        target_type: 'report',
        details: { targetType: 'human', targetId: bob.humanId },
      })
      // The reporter sees the new status in their history.
      expect((await myReports(db, alice.as)).find((r) => r.id === report.id)?.status).toBe(
        'dismissed',
      )
    })
    ...
    it('unknown reports and missing arguments are refused', async () => {
      expect(
        await errorCode(
          db.rpc('report_resolve', { report_id: randomUUID(), status: 'resolved' }, 'service'),
        ),
      ).toBe('not_visible')
      expect(
        await errorCode(
          db.rpc('report_resolve', { report_id: null, status: 'resolved' }, 'service'),
        ),
      ).toBe('invalid_input')
    })
    ...
    it('clients cannot execute it, and the role check refuses them even when granted', async () => {
      ...
      await db.sql.query(
        'grant execute on function public.report_resolve(uuid, public.report_status) to anon, authenticated',
      )
      try {
        expect(
          await errorCode(
            db.rpc('report_resolve', { report_id: report.id, status: 'resolved' }, alice.as),
          ),
        ).toBe('forbidden')
        expect(
          await errorCode(
            db.rpc('report_resolve', { report_id: report.id, status: 'resolved' }, 'visitor'),
          ),
        ).toBe('forbidden')
      } finally {
        await db.sql.query(
          'revoke execute on function public.report_resolve(uuid, public.report_status) from anon, authenticated',
        )
      }
  ...
  describe('row invariants (0700)', () => {
    it('identity columns are immutable; status and details may change', async () => {
      ...
      await db.sql.query(
        `update public.reports set details = 'edited by moderation' where id = $1`,
        [report.id],
      )
    ...
    it('a report survives its reporter: the reference is cleared, the kind is kept', async () => {
      ...
      const report = await createReport(db, throwaway.as, {
        targetType: 'human',
        targetId: bob.humanId,
      })
    ...
    it('every foreign key and query path has an index', async () => {
      const { rows } = await db.sql.query<{ indexdef: string }>(
        `select indexdef from pg_indexes where schemaname = 'public' and tablename = 'reports'`,
      )
      ...
      for (const column of [
        ...
        expect(
          defs.some((d) => d.includes(`(${column}`)),
          column,
        ).toBe(true)
      }
      expect(
        await db.sql
          .query(`select relrowsecurity from pg_class where oid = 'public.reports'::regclass`)
          .then((r) => r.rows[0]?.relrowsecurity),
      ).toBe(true)
    })
  })
})
