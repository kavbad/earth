...
describe('RLS matrix: posts, post_media, post_reactions, post_hides', () => {
  ...
  beforeAll(async () => {
    ...
    selfFriends = (await createPost(db, self, { type: 'image', text: 'friends', audience: 'friends', media: [await createMedia(db, self)] })).post.id
    selfWorld = (await createPost(db, self, { type: 'image', text: 'world', audience: 'world', media: [await createMedia(db, self)] })).post.id
    ...
    const blockedWorld = (await createPost(db, blocked, { type: 'image', text: 'blocked world', audience: 'world', media: [await createMedia(db, blocked)] })).post.id
  ...
  const cases = (): TableCase[] => [
    {
      ...
      select: { visitor: 3, guest: 3, claiming: 3, self: 6, other: 3, friend: 6, blocked: 2, member: 4 },
    ...
    },
    {
      ...
      select: { visitor: 2, guest: 2, claiming: 2, self: 2, other: 2, friend: 3, blocked: 1, member: 2 },
    ...
    },
    {
      ...
      select: { visitor: 4, guest: 4, claiming: 4, self: 4, other: 4, friend: 5, blocked: 1, member: 4 },
    ...
    {
      table: 'post_hides',
      select: { visitor: 'denied', guest: 0, claiming: 0, self: 1, other: 0, friend: 1, blocked: 0, member: 0 },
  ...
  it('the service role reads every row', async () => {
    const asService = await db.asRole('service', (c) => c.query('select count(*)::int as n from public.posts'))
