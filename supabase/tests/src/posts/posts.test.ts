...
describe('posts (DB_API §4)', () => {
  ...
  describe('post_create', () => {
    it('returns a PostViewDto with author, counts, empty media and no place', async () => {
      ...
      expect(await postRow(db, view.post.id)).toMatchObject({
        status: 'active',
        audience: 'friends',
      })
    })
    ...
    it('validates content: text or media, length, type consistency, media ownership and bucket', async () => {
      await db.expectError(
        db.rpc('post_create', postArgs({ text: '   ' }), alice.as),
        'invalid_input',
      )
      await db.expectError(
        db.rpc('post_create', postArgs({ text: null }), alice.as),
        'invalid_input',
      )
      await db.expectError(
        db.rpc('post_create', postArgs({ text: 'x'.repeat(2001) }), alice.as),
        'invalid_input',
      )
      const ok = await createPost(db, alice, { text: 'x'.repeat(2000) })
      expect(ok.post.text?.length).toBe(2000)
      await db.expectError(
        db.rpc('post_create', postArgs({ type: 'image', text: 'no media' }), alice.as),
        'invalid_input',
      )
      await db.expectError(
        db.rpc('post_create', postArgs({ type: 'video', text: null }), alice.as),
        'invalid_input',
      )
      const mine = await createMedia(db, alice)
      // A text post carries no media; media posts carry media.
      await db.expectError(
        db.rpc('post_create', postArgs({ type: 'text', text: 'hi', media: [mine] }), alice.as),
        'invalid_input',
      )
      const bobs = await createMedia(db, bob)
      await db.expectError(
        db.rpc('post_create', postArgs({ type: 'image', text: null, media: [bobs] }), alice.as),
        'invalid_input',
      )
      const avatar = await createMedia(db, alice, { bucket: 'avatars', key: 'alice/avatar.jpg' })
      await db.expectError(
        db.rpc('post_create', postArgs({ type: 'image', text: null, media: [avatar] }), alice.as),
        'invalid_input',
      )
      const pdf = await createMedia(db, alice, {
        contentType: 'application/pdf',
        key: 'alice/doc.pdf',
      })
      await db.expectError(
        db.rpc('post_create', postArgs({ type: 'image', text: null, media: [pdf] }), alice.as),
        'invalid_input',
      )
      await db.expectError(
        db.rpc(
          'post_create',
          postArgs({ type: 'image', text: null, media: [mine, mine] }),
          alice.as,
        ),
        'invalid_input',
      )
      await db.expectError(
        db.rpc(
          'post_create',
          postArgs({ type: 'image', text: null, media: [mine], provenance: ['edited', 'edited'] }),
          alice.as,
        ),
        'invalid_input',
      )
      ...
      await db.expectError(
        db.rpc('post_create', postArgs({ type: 'image', text: null, media: eleven }), alice.as),
        'invalid_input',
      )
    })
    ...
    it('stores media in order with type from the object and provenance per item', async () => {
      ...
      const video = await createMedia(db, alice, {
        key: 'alice/b.mp4',
      ...
      expect(view.media.map((m) => PostMediaDtoSchema.parse(m).mediaType)).toEqual([
        'image',
        'video',
      ])
      ...
      expect(view.media[1]).toMatchObject({
        width: 0,
        height: 0,
        durationMs: 4200,
        provenance: 'uploaded',
      ...
      expect(view.media[0]?.url).toMatch(
        /^https:\/\/earth\.social\/api\/media\/media\/alice\/a\.jpg$/,
      )
      const noProvenance = await createPost(db, alice, {
        type: 'image',
        text: null,
        media: [await createMedia(db, alice)],
      })
    ...
    it('neighborhood and city posts take their area from human_context and never store coordinates', async () => {
      const neighborhood = await createPost(db, alice, {
        text: 'block party',
      ...
      const tagged = await createPost(db, alice, {
        text: 'in marina',
        audience: 'city',
        areaId: marina,
      })
      ...
      await db.expectError(
        db.rpc(
          'post_create',
          postArgs({ audience: 'world', areaId: '00000000-0000-4000-8000-000000000000' }),
          alice.as,
        ),
        'area_not_found',
      )
      ...
      await db.expectError(
        db.rpc('post_create', postArgs({ audience: 'neighborhood' }), carol.as),
        'area_not_found',
      )
      await db.expectError(
        db.rpc('post_create', postArgs({ audience: 'city' }), carol.as),
        'area_not_found',
      )
      ...
      expect((await createPost(db, carol, { text: 'derived', audience: 'city' })).post.areaId).toBe(
        sf,
      )
    ...
    it('audience flags gate root posts', async () => {
      ...
      await db.expectError(
        db.rpc('post_create', postArgs({ audience: 'neighborhood' }), alice.as),
        'feature_disabled',
      )
      await setFlag(db, 'NEIGHBORHOOD_ENABLED', true)
      await setFlag(db, 'CITY_ENABLED', false)
      await db.expectError(
        db.rpc('post_create', postArgs({ audience: 'city' }), alice.as),
        'feature_disabled',
      )
      await setFlag(db, 'CITY_ENABLED', true)
      await setFlag(db, 'WORLD_ENABLED', false)
      await db.expectError(
        db.rpc('post_create', postArgs({ audience: 'world' }), alice.as),
        'feature_disabled',
      )
    ...
    it('place tags are explicit: a public place resolves to a PlaceDto, an unknown or private one is rejected', async () => {
      ...
      await db.expectError(
        db.rpc(
          'post_create',
          postArgs({ placeId: '00000000-0000-4000-8000-000000000000' }),
          alice.as,
        ),
        'invalid_input',
      )
      ...
      await db.expectError(
        db.rpc('post_create', postArgs({ placeId: secret }), alice.as),
        'invalid_input',
      )
    })
    ...
    it('is rate limited to 20 posts per hour per Human', async () => {
      ...
      await db.expectError(
        db.rpc('post_create', postArgs({ text: 'one too many' }), prolific.as),
        'rate_limited',
      )
  ...
  describe('replies (spec §31, §72; SCREEN 07)', () => {
    it('a reply requested as world to a friends post is stored as friends — never audience_too_wide, never wider', async () => {
      ...
      const reply = await createPost(db, bob, {
        text: 'me too',
        audience: 'world',
    ...
    it('replies to replies keep the thread root and maintain reply_count on the direct parent', async () => {
      ...
      const first = await createPost(db, bob, {
        text: 'first',
        audience: 'world',
        parentPostId: root.post.id,
      })
      const nested = await createPost(db, carol, {
        text: 'nested',
        audience: 'world',
        parentPostId: first.post.id,
      })
    ...
    it('reply_policy is enforced against the root: none → reply_not_allowed, friends → friends only, mentioned admits nobody else', async () => {
      const closed = await createPost(db, alice, {
        text: 'no replies',
        audience: 'world',
        replyPolicy: 'none',
      })
      await db.expectError(
        db.rpc(
          'post_create',
          postArgs({ text: 'hey', audience: 'world', parentPostId: closed.post.id }),
          bob.as,
        ),
        'reply_not_allowed',
      )
      ...
      const own = await createPost(db, alice, {
        text: 'ps',
        audience: 'world',
        parentPostId: closed.post.id,
      })
      ...
      await db.expectError(
        db.rpc(
          'post_create',
          postArgs({ text: 'nested', audience: 'world', parentPostId: own.post.id }),
          bob.as,
        ),
        'reply_not_allowed',
      )

      const friendsOnly = await createPost(db, alice, {
        text: 'friends may reply',
        audience: 'world',
        replyPolicy: 'friends',
      })
      await db.expectError(
        db.rpc(
          'post_create',
          postArgs({ text: 'hey', audience: 'world', parentPostId: friendsOnly.post.id }),
          carol.as,
        ),
        'reply_not_allowed',
      )
      await createPost(db, bob, {
        text: 'friend here',
        audience: 'world',
        parentPostId: friendsOnly.post.id,
      })

      const mentioned = await createPost(db, alice, {
        text: 'mentions only',
        audience: 'world',
      ...
      await db.expectError(
        db.rpc(
          'post_create',
          postArgs({ text: 'hey', audience: 'world', parentPostId: mentioned.post.id }),
          bob.as,
        ),
        'reply_not_allowed',
      )
    ...
    it('replying needs a visible, active parent; blocks refuse', async () => {
      ...
      await db.expectError(
        db.rpc('post_create', postArgs({ text: 'hi', parentPostId: hidden.post.id }), carol.as),
        'post_not_found',
      )
      await db.expectError(
        db.rpc(
          'post_create',
          postArgs({ text: 'hi', parentPostId: '00000000-0000-4000-8000-000000000000' }),
          carol.as,
        ),
        'post_not_found',
      )
      ...
      await db.expectError(
        db.rpc('post_create', postArgs({ text: 'hi', parentPostId: removed.post.id }), bob.as),
        'post_not_found',
      )
      // The author can still see their removed post but cannot reply to it.
      await db.expectError(
        db.rpc('post_create', postArgs({ text: 'hi', parentPostId: removed.post.id }), alice.as),
        'post_not_found',
      )
      ...
      await db.expectError(
        db.rpc('post_create', postArgs({ text: 'hi', parentPostId: root.post.id }), carol.as),
        'post_not_found',
      )
    })
    ...
    it('post_replies pages direct replies oldest first with a stable cursor', async () => {
      ...
      const page1 = RepliesPageSchema.parse(
        await db.rpc('post_replies', { post_id: root.post.id, cursor: null, limit: 2 }, 'visitor'),
      )
      expect(page1.replies.map((r) => r.post.id)).toEqual(ids.slice(0, 2))
      expect(page1.nextCursor).toBe(ids[1])
      const page2 = RepliesPageSchema.parse(
        await db.rpc(
          'post_replies',
          { post_id: root.post.id, cursor: page1.nextCursor, limit: 2 },
          'visitor',
        ),
      )
      ...
      await db.expectError(
        db.rpc('post_replies', { post_id: root.post.id, cursor: 'nope', limit: 2 }, 'visitor'),
        'invalid_input',
      )
      await db.expectError(
        db.rpc(
          'post_replies',
          { post_id: root.post.id, cursor: root.post.id, limit: 2 },
          'visitor',
        ),
        'invalid_input',
      )
  ...
  describe('visibility — earth.can_view_post (spec §71; DB_API §4)', () => {
    it('a friends post is visible to the author and friends only: never to non-friends, visitors, guests, or a friend blocked either way', async () => {
      ...
      expect(
        await canSee(
          db,
          post.post.id,
          (await createHuman(db, { handle: 'claimingviewer', status: 'pending' })).as,
        ),
      ).toBe(false)
    ...
    it('a neighborhood post is visible in the area, in its children, to friends elsewhere; not to strangers in another neighborhood', async () => {
      ...
      const cityTagged = await createPost(db, alice, {
        text: 'anywhere in sf',
        audience: 'neighborhood',
        areaId: sf,
      })
    ...
    it('a world post is visible to everyone; visitors and guests only while PUBLIC_WORLD_ENABLED', async () => {
      ...
      const asVisitor = await db.asRole('visitor', (c) =>
        c.query('select id from public.posts where id = $1', [post.post.id]),
      )
      expect(asVisitor.rowCount).toBe(0)
      await setFlag(db, 'PUBLIC_WORLD_ENABLED', true)
      const again = await db.asRole('visitor', (c) =>
        c.query('select id from public.posts where id = $1', [post.post.id]),
      )
  ...
  describe('reactions, hides and deletion', () => {
    it('post_reaction_set upserts one reaction per Human and clears it with null', async () => {
      ...
      expect(
        await db.rpc(
          'post_reaction_set',
          { post_id: post.post.id, reaction_type: 'heart' },
          bob.as,
        ),
      ).toEqual({
        postId: post.post.id,
      ...
      expect(
        await db.rpc(
          'post_reaction_set',
          { post_id: post.post.id, reaction_type: ' fire ' },
          bob.as,
        ),
      ).toMatchObject({ myReaction: 'fire', reactionCount: 1 })
      expect(
        await db.rpc(
          'post_reaction_set',
          { post_id: post.post.id, reaction_type: 'heart' },
      ...
      expect(
        await db.rpc('post_reaction_set', { post_id: post.post.id, reaction_type: null }, bob.as),
      ).toMatchObject({ myReaction: null, reactionCount: 1 })
      expect(
        await db.rpc('post_reaction_set', { post_id: post.post.id, reaction_type: '' }, bob.as),
      ).toMatchObject({ myReaction: null, reactionCount: 1 })
      await db.expectError(
        db.rpc(
          'post_reaction_set',
          { post_id: post.post.id, reaction_type: 'x'.repeat(17) },
          bob.as,
        ),
        'invalid_input',
      )
      await db.expectError(
        db.rpc('post_reaction_set', { post_id: post.post.id, reaction_type: 'heart' }, 'visitor'),
        'not_authenticated',
      )
      await db.expectError(
        db.rpc(
      ...
      await db.expectError(
        db.rpc(
          'post_reaction_set',
          { post_id: friendsPost.post.id, reaction_type: 'heart' },
          carol.as,
        ),
        'post_not_found',
      )
      await db.expectError(
        db.rpc(
          'post_reaction_set',
          { post_id: '00000000-0000-4000-8000-000000000000', reaction_type: 'heart' },
          bob.as,
        ),
        'post_not_found',
      )
    ...
    it('post_hide is idempotent, needs a visible post and never hides the post from a direct fetch', async () => {
      ...
      expect(await db.rpc('post_hide', { post_id: post.post.id }, bob.as)).toEqual({
        postId: post.post.id,
        hidden: true,
      })
      expect(await db.rpc('post_hide', { post_id: post.post.id }, bob.as)).toEqual({
      ...
      expect(await canSee(db, post.post.id, bob.as)).toBe(true)
      await db.expectError(
        db.rpc('post_hide', { post_id: post.post.id }, 'visitor'),
        'not_authenticated',
      )
      const friendsPost = await createPost(db, alice, { text: 'friends', audience: 'friends' })
      await db.expectError(
        db.rpc('post_hide', { post_id: friendsPost.post.id }, carol.as),
        'post_not_found',
      )
    })
    ...
    it('post_delete is a soft delete by the author (or the service): content leaves distribution at once', async () => {
      ...
      const post = await createPost(db, alice, {
        type: 'image',
        text: 'bye',
        audience: 'world',
        media: [media],
      ...
      await db.expectError(
        db.rpc('post_delete', { post_id: post.post.id }, 'visitor'),
        'not_authenticated',
      )
      const friendsPost = await createPost(db, alice, { text: 'friends', audience: 'friends' })
      await db.expectError(
        db.rpc('post_delete', { post_id: friendsPost.post.id }, carol.as),
        'post_not_found',
      )
      ...
      await expect(
        db.sql.query(`update public.posts set status = 'active', deleted_at = null where id = $1`, [
          post.post.id,
        ]),
      ).rejects.toThrow('invalid_input')
      await expect(
        db.sql.query(`update public.posts set text = 'back' where id = $1`, [post.post.id]),
      ...
      await db.expectError(
        db.rpc('post_delete', { post_id: '00000000-0000-4000-8000-000000000000' }, 'service'),
        'post_not_found',
      )
    })
    ...
    it('post_get needs a visible post', async () => {
      await db.expectError(
        db.rpc('post_get', { post_id: '00000000-0000-4000-8000-000000000000' }, alice.as),
        'post_not_found',
      )
