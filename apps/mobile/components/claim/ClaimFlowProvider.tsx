 ...
 * screen reads `useClaimFlow()`; the guard here keeps the screen and the step in agreement.
 */
import { CLAIM_FLAG_KEY, type ClaimFlags, type ClaimFlowState } from '@earth/auth'
import type { ClaimIntent, EarthErrorCode } from '@earth/domain'
...
import { useAnalytics, useEarth, useFlags, useSession, useToast } from '@/lib/providers'
...
export function ClaimFlowProvider({ children }: { readonly children: ReactNode }) {
  ...
  /**
   * Dispatches to the machine and, once the flow is resolved, opens the screen of a step the
   * event moved forward to (`routeAfterAdvance`); the guard below never navigates forward itself.
   */
  ...
  const continueAsHuman = useCallback(
    async (pending: PendingClaim | null): Promise<void> => {
      ...
      try {
        ...
        const code = errorCode(cause)
        toast.show(code.startsWith('invite_') ? shellCopy.inviteInvalid : shellCopy.somethingWrong)
      }
  ...
  const resolveCredential = useCallback(async (): Promise<StartClaimResult> => {
    ...
    const pending = await readPendingClaim(deviceStorage())
    ...
    if (intent === null) {
      // Nothing chosen yet: the gate comes first (spec §44).
  ...
  // Remember a Visitor's choices so a relaunch resumes where they were.
  useEffect(() => {
    if (!ready || state.intent === null || state.authenticated) return
    void writePendingClaim(deviceStorage(), pendingFromState(state, startedAt.current))
  }, [ready, state])

  // The screen follows the step: nobody skips ahead, nobody goes back past the credential.
  useEffect(() => {
    if (!ready) return
