/**
 * Shell glue around the `@earth/auth` claim state machine (spec §44–§49, §111): which route each
 ...
 * Everything here is pure; the screens dispatch and navigate. Mirrors the web client's
 * `lib/claim/flow.ts` with string routes and an asynchronous device store.
 */
...
import { ROUTES, conversationRoute } from '../routes'
...
export function routeForStep(step: ClaimStep): string {
  switch (step) {
    case 'gate':
      return ROUTES.claim
    case 'group_label':
      return ROUTES.claimStart
    case 'credential':
      return ROUTES.claimCredential
    case 'identity':
      return ROUTES.claimIdentity
    case 'human_pass':
    case 'verifying':
    case 'duplicate':
    case 'help':
    case 'complete':
      return ROUTES.claimHuman
    case 'done':
...
/** The step a claim screen stands for; `/claim/join` is the transitional entry of spec §46. */
...
/**
 * Where to send someone whose screen does not fit the claim state, or `null` when the screen may
 ...
 */
export function claimRedirectFor(state: ClaimFlowState, pathname: string): string | null {
  const at = stepForPathname(pathname)
  if (at === null) return null
  const current = stepRank(state.step)
  const here = stepRank(at)
  if (state.step === ClaimSteps.done) return at === ClaimSteps.done ? null : ROUTES.welcome
...
/**
 * The screen to open after an event moved the claim forward (`chooseStart` → the label,
 * `labelSet` → the credential, `claim_start` → identity, `identitySet` → Human Pass), or `null`
 * when the person stays where they are.
 */
export function routeAfterAdvance(previous: ClaimFlowState, next: ClaimFlowState): string | null {
...
export function readPendingClaim(store: KeyValueStorage | null): Promise<PendingClaim | null> {
...
export function writePendingClaim(
  ...
  return writeJson(store, PENDING_CLAIM_KEY, pending)
}

export function clearPendingClaim(store: KeyValueStorage | null): Promise<void> {
  return removeKey(store, PENDING_CLAIM_KEY)
}
...
export function completionRecord(
  ...
  intent: ClaimIntent | null,
): ClaimCompletionRecord {
  return { ...completion, intent }
}

/** Spec §46 step 8 / §49: the group's conversation, never a generic Home. */
export function destinationAfterClaim(
  completion: Pick<ClaimCompleteDto, 'conversationId'>,
): string {
