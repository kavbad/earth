...
export function ScopeProvider({ children }: { readonly children: ReactNode }) {
  ...
  const roleKind = session.roleKind
  const humanId = session.humanId
  useEffect(() => {
    if (sessionStatus !== 'ready') return
    let cancelled = false
    void readStoredScopes(deviceStorage(), humanId).then((stored) => {
      if (cancelled) return
      dispatch({ type: 'reset', scopes: initialScopes({ roleKind, stored, flags }) })
    })
    return () => {
      cancelled = true
    }
  }, [sessionStatus, roleKind, humanId, flags])
  ...
  const setScope = useCallback(
    (surface: ScopeSurface, scope: Scope) => {
      ...
      void rememberScope(deviceStorage(), surface, session.humanId, scope)
