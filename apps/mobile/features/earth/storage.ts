/**
 * Guarded key-value storage for the Earth feature (own location shares, device preferences,
 * notification categories). Asynchronous because the device store is AsyncStorage; tests and
 * runtimes without a store use the in-memory implementation. Nothing here imports React Native —
 * the AsyncStorage adapter lives in `deviceStorage.ts`. Persistence is a convenience, never a
 * requirement (spec §107 keeps the app usable regardless).
 */
