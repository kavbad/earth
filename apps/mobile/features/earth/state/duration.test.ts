...
const at = (hour: number, minute = 0): Date => new Date(2026, 8, 3, hour, minute, 0, 0)
...
describe('share durations (spec §75)', () => {
  ...
  it('"Tonight" runs until the next early morning and never past the maximum', () => {
    ...
    expect(tonightMinutes(at(2))).toBe(Math.min(DAY_MINUTES, LOCATION_SHARE_MAX_MINUTES))
  ...
  it('never yields forever or nothing: custom values are clamped to the domain bounds (≤ 24 h)', () => {
    expect(clampMinutes(0)).toBe(LOCATION_SHARE_MIN_MINUTES)
    expect(clampMinutes(10_000)).toBe(LOCATION_SHARE_MAX_MINUTES)
    expect(LOCATION_SHARE_MAX_MINUTES).toBeLessThanOrEqual(DAY_MINUTES)
    expect(durationMinutesFor({ kind: 'custom', customMinutes: 90 }, at(9))).toBe(90)
    expect(
      durationMinutesFor({ kind: 'custom', customMinutes: Number.POSITIVE_INFINITY }, at(9)),
    ).toBe(LOCATION_SHARE_MAX_MINUTES)
    expect(
  ...
  it('validates a typed value against the exact rule', () => {
    ...
    expect(validateDurationMinutes(DAY_MINUTES + 1)).toEqual({ ok: false, reason: 'too_long' })
