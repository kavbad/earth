...
describe('display identity form', () => {
  it('tracks edits, saving and errors', () => {
    ...
    form = identityFormReducer(form, { type: 'reset', displayName: 'M', bio: 'b' })
    expect(form).toEqual(initialIdentityForm('M', 'b'))
  })
  ...
  it('sends only the fields that changed', () => {
    ...
    ).toEqual({ displayName: 'Maya R', bio: 'Hi' })
  })
})
...
describe('credentials', () => {
  it('reads email and phone off the session', () => {
    ...
    ).toEqual({ email: 'a@b.co', phone: null })
    expect(
      credentialMethod({ access_token: 't', user: { id: 'u', email: 'a@b.co', phone: '+1' } }),
    ).toBe('phone')
  ...
  it('walks enter → code → done and restarts cleanly', () => {
    ...
    expect(credentialFlowReducer(flow, { type: 'start', method: 'email' })).toEqual(
      initialCredentialFlow('email'),
    )
  })
...
describe('one-shot requests (recovery, help)', () => {
  it('tracks which request is busy, which one is done and the last error', () => {
    let state = initialRequestState<'help' | 'recovery'>()
    state = requestReducer(state, { type: 'start', kind: 'help' })
    expect(state).toEqual({ busy: 'help', done: null, error: null })
    state = requestReducer(state, { type: 'failed', error: 'nope' })
    expect(state).toEqual({ busy: null, done: null, error: 'nope' })
    state = requestReducer(state, { type: 'start', kind: 'recovery' })
    state = requestReducer(state, { type: 'done', kind: 'recovery' })
    expect(state).toEqual({ busy: null, done: 'recovery', error: null })
    expect(requestReducer(state, { type: 'reset' })).toEqual(initialRequestState())
  })
})
