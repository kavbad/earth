 ...
 * machine (stale answers never win), the add-a-credential flow and the delete request.
 * Reducers so the screens stay thin and the rules are unit-tested.
 */
...
export function credentialMethod(session: AuthSessionLike | null): CredentialMethod | null {
  const credentials = credentialsFrom(session)
  if (credentials.phone !== null) return 'phone'
  if (credentials.email !== null) return 'email'
  return null
}
...
/** The OTP is at least this long before "Continue" is offered. */
...
// ---------------------------------------------------------------------------
// A small async-action machine shared by the one-shot requests (recovery, help, delete)
// ---------------------------------------------------------------------------

export interface RequestState<K extends string> {
  readonly busy: K | null
  readonly done: K | null
  readonly error: string | null
}

export type RequestAction<K extends string> =
  | { readonly type: 'start'; readonly kind: K }
  | { readonly type: 'done'; readonly kind: K }
  | { readonly type: 'failed'; readonly error: string }
  | { readonly type: 'reset' }

export function initialRequestState<K extends string>(): RequestState<K> {
  return { busy: null, done: null, error: null }
}

export function requestReducer<K extends string>(
  state: RequestState<K>,
  action: RequestAction<K>,
): RequestState<K> {
  switch (action.type) {
    case 'start':
      return { busy: action.kind, done: state.done, error: null }
    case 'done':
      return { busy: null, done: action.kind, error: null }
    case 'failed':
      return { busy: null, done: state.done, error: action.error }
    case 'reset':
      return initialRequestState()
    default: {
      const exhaustive: never = action
      throw new Error(`Unknown request action: ${String(exhaustive)}`)
    }
  }
}
