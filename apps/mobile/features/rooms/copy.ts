/**
 * Mobile-only microcopy for the Live and room surfaces at moments the spec leaves unnamed.
 * Everything the spec quotes (SCREEN 13–16, §59, §61, §109) comes from `@earth/ui`'s `copy` and is
 * never restated here; the values below are labels, hints and error lines around them. Mirrors the
 * web client's `components/rooms/copy.ts` so both clients read the same at the same moments.
 */
...
export const roomCopy = {
  ...
  // Room link preview (`/live/:token` opened in the app)
  invitedBy: (name: string): string => `Shared by ${name}`,
  linkNotUsable: "This link doesn't lead anywhere anymore.",
  openOnWeb: 'Open on the web to join as a Guest',
  guestsOnWeb: 'Guests join from the web.',
  joining: 'Joining…',

  // Shell seam (until the app shell provides these)
  envMissing: 'Earth is not configured for this build.',
