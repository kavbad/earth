...
/** Publishing Humans whose consent does not yet cover the pending visibility (SCREEN 15 status line). */
export function pendingConsentCount(
  room: Pick<RoomDto, 'pendingVisibility' | 'participants'>,
): number {
...
/** Active viewers (`watching`) — the small audience indicator next to the context title. */
export function watchingCount(room: Pick<RoomDto, 'participants'>): number {
  return room.participants.filter((p) => p.status === 'active' && p.mediaState === 'watching')
    .length
}
