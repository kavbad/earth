...
describe('openUpFormReducer', () => {
  it('re-derives the join policy when the visibility changes', () => {
    const reduce = openUpFormReducer('standalone')
    const start = { visibility: 'friends', joinPolicy: 'friends' } as const
    expect(reduce(start, { type: 'visibility', visibility: 'invited' })).toEqual({
      visibility: 'invited',
      joinPolicy: 'invited_only',
    })
    expect(reduce(start, { type: 'visibility', visibility: 'world' })).toEqual({
      visibility: 'world',
      joinPolicy: 'friends',
    })
    expect(reduce(start, { type: 'joinPolicy', joinPolicy: 'request' })).toEqual({
      visibility: 'friends',
      joinPolicy: 'request',
    })
  })
