 ...
 * relationship. A profile opened from a card starts `profile_get` on arrival, and a tap on
 * `Add Friend` inside that window lands while the read is still in flight carrying
 * `friendRequest: 'none'`. React-query writes a late answer over a cache write, so the recorded
 * request used to disappear behind `Add Friend` again with nothing left to refetch it.
 */
...
describe('commitProfile (SCREEN 22)', () => {
  it('holds the relationship answer against a profile_get that was already in flight', async () => {
    ...
    // The arrival read does not observe the abort signal (`profile_get` is a plain POST), so it
    // settles regardless — the whole point of the race.
