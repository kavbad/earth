 ...
 * renders pending → sent / failed, a non-retryable server answer fails at once, and the queue
 * persists per Human and conversation through the asynchronous device store.
 */
...
import { createFakeClock, flushPromises } from '@earth/realtime/testing'
...
function harness(
  options: {
    readonly sendFails?: 'blocked' | 'internal' | null
    readonly store?: ReturnType<typeof createMemoryStorage>
  } = {},
): Harness {
  ...
  const store = options.store ?? createMemoryStorage()
  const result: Harness = {
    online: true,
    store,
    outbox: createOutbox({
      // The device store: AsyncStorage in the app, memory here — both asynchronous.
      storage: createOutboxStorage(store, XAVIER, CONVERSATION),
...
describe('chat outbox integration', () => {
  ...
  it('persists the queue per Human and conversation and resumes it after a relaunch', async () => {
    ...
    // A killed app: a fresh outbox over the same store finds the item and sends it once online.
