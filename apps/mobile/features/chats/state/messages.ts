 ...
 *
 * Pure so it is unit-tested without React (same rules as the web client).
 */
...
/** A group never grows past this many messages, so a list row stays small. */
...
/**
 * The rows an inverted list renders: newest first, so index 0 sits at the visual bottom. The
 * day label of each row still belongs above that row, whichever direction the list runs.
 */
export function invertRows<T>(rows: readonly T[]): T[] {
  const out: T[] = []
