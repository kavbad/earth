 ...
 * object; the clients pin these shapes with zod and parse leniently on read (an unknown shape
 * renders as its type's preview, never crashes). Same shapes as the web client writes.
 *
 ...
 * - system: `{ kind, actorHumanId }` written by the database.
 */
...
/** Trimmed, non-empty poll options (what the composer sends). */
...
/** Mirror of `earth.message_preview(type, text)`: the row line for non-text messages. */
...
/** A content type `media.upload` accepts; pickers report `undefined` / `''` for unknown files. */
...
/**
 * Content type of a picked asset: the picker's `mimeType` when it has one, else a guess from the
 * file name / uri extension, else the picker's kind (`image` → `image/jpeg`, `video` → `video/mp4`).
 */
export function contentTypeForAsset(asset: {
  ...
  const direct = normalizeContentType(asset.mimeType)
  if (direct !== FALLBACK_CONTENT_TYPE) return direct
  const name = asset.fileName ?? asset.uri.split('?')[0] ?? ''
  const extension = name.split('.').pop()?.toLowerCase() ?? ''
  const guessed = EXTENSION_CONTENT_TYPES[extension]
