...
/**
 * Halo radius (px) around a shared friend, from the precision: a City share is a wide, soft
 * area; approximate a smaller one; precise none. Never a colored border around the face.
 */
export const FRIEND_HALO_PX = {
  city: 72,
  approximate: 48,
  precise: 0,
} as const satisfies Record<LocationPrecision, number>

export function friendHaloPx(precision: LocationPrecision): number {
  return FRIEND_HALO_PX[precision]
}
