...
/** The audience this Human last posted to on this device, or `null`. */
export function readLastAudience(
  storage: KeyValueStorage | null,
  humanId: string | null,
): Audience | null {
  if (humanId === null) return null
  const parsed = AudienceSchema.safeParse(readString(storage, lastAudienceStorageKey(humanId)))
...
export function rememberLastAudience(
  ...
  writeString(storage, lastAudienceStorageKey(humanId), audience)
}
...
export function initialComposerAudience(input: {
  readonly requested: Audience | null
  readonly last: Audience | null
  readonly cap: Audience | null
